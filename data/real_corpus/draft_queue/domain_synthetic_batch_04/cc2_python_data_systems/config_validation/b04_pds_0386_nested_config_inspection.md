...
---
# Config Validation: Nested Config Inspection with Failure Diagnosis Pattern

## Teaching Objective
This draft teaches explaining nested config inspection through a failure diagnosis pattern for config validation.
...
## Concept
The core idea is nested config inspection.
...
## Minimal Example
```text
input: b04-pds-0386 sample row
focus: nested config inspection
...
- Confirm the example describes nested config inspection rather than a different subtopic.
...
Its purpose is to give reviewers a single teaching target: explaining nested config inspection through a failure diagnosis pattern for config validation.
...
## Variation Note
Local example index: 86 within the config validation block.
