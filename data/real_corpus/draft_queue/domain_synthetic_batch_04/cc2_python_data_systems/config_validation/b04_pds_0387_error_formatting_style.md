...
---
# Config Validation: Error Formatting Style with Failure Diagnosis Pattern

## Teaching Objective
This draft teaches explaining error formatting style through a failure diagnosis pattern for config validation.
...
## Concept
The core idea is error formatting style.
...
## Minimal Example
```text
input: b04-pds-0387 sample row
focus: error formatting style
...
- Confirm the example describes error formatting style rather than a different subtopic.
...
Its purpose is to give reviewers a single teaching target: explaining error formatting style through a failure diagnosis pattern for config validation.
...
## Variation Note
Local example index: 87 within the config validation block.
