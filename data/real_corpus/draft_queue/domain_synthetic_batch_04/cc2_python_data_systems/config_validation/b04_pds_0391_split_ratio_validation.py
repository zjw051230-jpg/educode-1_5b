...
def build_demo_rows() -> list[dict[str, object]]:
    return [
        {"topic_id": "B04-PDS-0391", "label": EXAMPLE_LABELS[0], "position": 0},
        {"topic_id": "B04-PDS-0391", "label": EXAMPLE_LABELS[1], "position": 1},
        {"topic_id": "B04-PDS-0391", "label": EXAMPLE_LABELS[2], "position": 2},
    ]
...
def summarize_split_ratio_validation(rows: Iterable[dict[str, object]]) -> dict[str, object]:
    ...
    return {
        "topic_id": "B04-PDS-0391",
        "objective": "explaining split ratio validation through a failure diagnosis pattern for config validation",
...
def render_demo_path(base: str | Path) -> Path:
    return Path(base) / "b04_pds_0391.jsonl"
