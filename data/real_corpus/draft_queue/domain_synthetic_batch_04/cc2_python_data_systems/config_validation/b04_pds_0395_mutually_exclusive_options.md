...
---
# Config Validation: Mutually Exclusive Options with Failure Diagnosis Pattern

## Teaching Objective
This draft teaches explaining mutually exclusive options through a failure diagnosis pattern for config validation.
...
## Concept
The core idea is mutually exclusive options.
...
## Minimal Example
```text
input: b04-pds-0395 sample row
focus: mutually exclusive options
...
- Confirm the example describes mutually exclusive options rather than a different subtopic.
...
Its purpose is to give reviewers a single teaching target: explaining mutually exclusive options through a failure diagnosis pattern for config validation.
...
## Variation Note
Local example index: 95 within the config validation block.
