...
---
# Config Validation: Draft Config Provenance with Failure Diagnosis Pattern

## Teaching Objective
This draft teaches explaining draft config provenance through a failure diagnosis pattern for config validation.
...
## Concept
The core idea is draft config provenance.
...
## Minimal Example
```text
input: b04-pds-0400 sample row
focus: draft config provenance
...
- Confirm the example describes draft config provenance rather than a different subtopic.
...
Its purpose is to give reviewers a single teaching target: explaining draft config provenance through a failure diagnosis pattern for config validation.
...
## Variation Note
Local example index: 100 within the config validation block.
