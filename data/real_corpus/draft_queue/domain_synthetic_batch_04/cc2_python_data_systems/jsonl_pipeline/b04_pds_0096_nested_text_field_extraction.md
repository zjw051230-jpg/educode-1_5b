...
---
# JSONL Pipeline: Nested Text Field Extraction with Failure Diagnosis Pattern

## Teaching Objective
This draft teaches explaining nested text field extraction through a failure diagnosis pattern for jsonl pipeline.
...
## Concept
The core idea is nested text field extraction.
...
## Minimal Example
```text
input: b04-pds-0096 sample row
focus: nested text field extraction
...
- Confirm the example describes nested text field extraction rather than a different subtopic.
...
Its purpose is to give reviewers a single teaching target: explaining nested text field extraction through a failure diagnosis pattern for jsonl pipeline.
...
## Variation Note
Local example index: 96 within the jsonl pipeline block.
