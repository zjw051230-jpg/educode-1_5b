...
---
# Provenance Metadata: External Text Prohibition Markers with Failure Diagnosis Pattern

## Teaching Objective
This draft teaches explaining external text prohibition markers through a failure diagnosis pattern for provenance metadata.
It keeps the focus on provenance metadata behavior instead of production scale concerns.
...
## Concept
The core idea is external text prohibition markers.
...
This draft highlights the narrower lens of failure diagnosis pattern so the reader sees one concrete decision boundary instead of a vague overview.
...
## Minimal Example
```text
input: b04-pds-0890 sample row
focus: external text prohibition markers
check: failure diagnosis pattern
expected_result: clear draft candidate output
```

## Review Checklist
- Confirm the metadata header keeps `approved_for_training: false`.
- Confirm the example describes external text prohibition markers rather than a different subtopic.
...
The third debugging question is whether the explanation still matches the directory theme of Provenance Metadata.

## Why This Draft Exists
This file is one of many small synthetic examples in domain_synthetic_batch_04.
Its purpose is to give reviewers a single teaching target: explaining external text prohibition markers through a failure diagnosis pattern for provenance metadata.
...
## Variation Note
Local example index: 90 within the provenance metadata block.
