...
---
# Sanity Checks: Metadata Header Presence with Failure Diagnosis Pattern

## Teaching Objective
This draft teaches explaining metadata header presence through a failure diagnosis pattern for sanity checks.
...
## Concept
The core idea is metadata header presence.
...
## Minimal Example
```text
input: b04-pds-0984 sample row
focus: metadata header presence
...
- Confirm the example describes metadata header presence rather than a different subtopic.
...
Its purpose is to give reviewers a single teaching target: explaining metadata header presence through a failure diagnosis pattern for sanity checks.
...
## Variation Note
Local example index: 84 within the sanity checks block.
