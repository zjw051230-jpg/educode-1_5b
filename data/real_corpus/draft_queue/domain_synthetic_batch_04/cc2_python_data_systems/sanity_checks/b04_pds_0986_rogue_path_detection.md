...
---
# Sanity Checks: Rogue Path Detection with Failure Diagnosis Pattern

## Teaching Objective
This draft teaches explaining rogue path detection through a failure diagnosis pattern for sanity checks.
...
## Concept
The core idea is rogue path detection.
...
## Minimal Example
```text
input: b04-pds-0986 sample row
focus: rogue path detection
...
- Confirm the example describes rogue path detection rather than a different subtopic.
...
Its purpose is to give reviewers a single teaching target: explaining rogue path detection through a failure diagnosis pattern for sanity checks.
...
## Variation Note
Local example index: 86 within the sanity checks block.
