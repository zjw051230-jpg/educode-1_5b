...
---
# Sanity Checks: Row Count Sanity Checks with Failure Diagnosis Pattern

## Teaching Objective
This draft teaches explaining row count sanity checks through a failure diagnosis pattern for sanity checks.
...
## Concept
The core idea is row count sanity checks.
...
## Minimal Example
```text
input: b04-pds-0987 sample row
focus: row count sanity checks
...
- Confirm the example describes row count sanity checks rather than a different subtopic.
...
Its purpose is to give reviewers a single teaching target: explaining row count sanity checks through a failure diagnosis pattern for sanity checks.
...
## Variation Note
Local example index: 87 within the sanity checks block.
