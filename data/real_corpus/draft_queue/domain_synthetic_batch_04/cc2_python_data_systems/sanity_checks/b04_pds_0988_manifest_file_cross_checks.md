...
---
# Sanity Checks: Manifest File Cross Checks with Failure Diagnosis Pattern

## Teaching Objective
This draft teaches explaining manifest file cross checks through a failure diagnosis pattern for sanity checks.
...
## Concept
The core idea is manifest file cross checks.
...
## Minimal Example
```text
input: b04-pds-0988 sample row
focus: manifest file cross checks
...
- Confirm the example describes manifest file cross checks rather than a different subtopic.
...
Its purpose is to give reviewers a single teaching target: explaining manifest file cross checks through a failure diagnosis pattern for sanity checks.
...
## Variation Note
Local example index: 88 within the sanity checks block.
