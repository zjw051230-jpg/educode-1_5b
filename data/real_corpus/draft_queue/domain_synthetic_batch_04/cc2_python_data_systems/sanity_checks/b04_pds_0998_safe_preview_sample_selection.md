...
---
# Sanity Checks: Safe Preview Sample Selection with Failure Diagnosis Pattern

## Teaching Objective
This draft teaches explaining safe preview sample selection through a failure diagnosis pattern for sanity checks.
...
## Concept
The core idea is safe preview sample selection.
...
## Minimal Example
```text
input: b04-pds-0998 sample row
focus: safe preview sample selection
...
- Confirm the example describes safe preview sample selection rather than a different subtopic.
...
Its purpose is to give reviewers a single teaching target: explaining safe preview sample selection through a failure diagnosis pattern for sanity checks.
...
## Variation Note
Local example index: 98 within the sanity checks block.
