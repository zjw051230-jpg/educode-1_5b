...
---
# Sanity Checks: Path Normalization Checks with Failure Diagnosis Pattern

## Teaching Objective
This draft teaches explaining path normalization checks through a failure diagnosis pattern for sanity checks.
...
## Concept
The core idea is path normalization checks.
...
## Minimal Example
```text
input: b04-pds-0999 sample row
focus: path normalization checks
...
- Confirm the example describes path normalization checks rather than a different subtopic.
...
Its purpose is to give reviewers a single teaching target: explaining path normalization checks through a failure diagnosis pattern for sanity checks.
...
## Variation Note
Local example index: 99 within the sanity checks block.
