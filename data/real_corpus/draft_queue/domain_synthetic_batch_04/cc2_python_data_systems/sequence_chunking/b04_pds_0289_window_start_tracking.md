...
---
# Sequence Chunking: Window Start Tracking with Failure Diagnosis Pattern

## Teaching Objective
This draft teaches explaining window start tracking through a failure diagnosis pattern for sequence chunking.
...
## Concept
The core idea is window start tracking.
...
## Minimal Example
```text
input: b04-pds-0289 sample row
focus: window start tracking
...
- Confirm the example describes window start tracking rather than a different subtopic.
...
Its purpose is to give reviewers a single teaching target: explaining window start tracking through a failure diagnosis pattern for sequence chunking.
...
## Variation Note
Local example index: 89 within the sequence chunking block.
