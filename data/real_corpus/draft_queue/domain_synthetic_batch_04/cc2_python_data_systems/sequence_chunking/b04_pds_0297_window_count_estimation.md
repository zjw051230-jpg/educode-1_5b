...
---
# Sequence Chunking: Window Count Estimation with Failure Diagnosis Pattern

## Teaching Objective
This draft teaches explaining window count estimation through a failure diagnosis pattern for sequence chunking.
...
## Concept
The core idea is window count estimation.
...
## Minimal Example
```text
input: b04-pds-0297 sample row
focus: window count estimation
...
- Confirm the example describes window count estimation rather than a different subtopic.
...
Its purpose is to give reviewers a single teaching target: explaining window count estimation through a failure diagnosis pattern for sequence chunking.
...
## Variation Note
Local example index: 97 within the sequence chunking block.
