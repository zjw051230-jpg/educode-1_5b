...
---
# Split Management: Minimum Validation Size with Failure Diagnosis Pattern

## Teaching Objective
This draft teaches explaining minimum validation size through a failure diagnosis pattern for split management.
...
## Concept
The core idea is minimum validation size.
...
## Minimal Example
```text
input: b04-pds-0784 sample row
focus: minimum validation size
...
- Confirm the example describes minimum validation size rather than a different subtopic.
...
Its purpose is to give reviewers a single teaching target: explaining minimum validation size through a failure diagnosis pattern for split management.
...
## Variation Note
Local example index: 84 within the split management block.
