...
---
# Split Management: Split Manifest Generation with Failure Diagnosis Pattern

## Teaching Objective
This draft teaches explaining split manifest generation through a failure diagnosis pattern for split management.
...
## Concept
The core idea is split manifest generation.
...
## Minimal Example
```text
input: b04-pds-0787 sample row
focus: split manifest generation
...
- Confirm the example describes split manifest generation rather than a different subtopic.
...
Its purpose is to give reviewers a single teaching target: explaining split manifest generation through a failure diagnosis pattern for split management.
...
## Variation Note
Local example index: 87 within the split management block.
