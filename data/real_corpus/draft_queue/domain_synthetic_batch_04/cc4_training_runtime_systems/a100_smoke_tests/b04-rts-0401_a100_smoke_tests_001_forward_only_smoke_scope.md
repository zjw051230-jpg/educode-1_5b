...
# Forward-Only Smoke Scope Review Notes

## Concept
Forward-only smoke scope is the main teaching target for this draft candidate.
...
## Explanation
Example index 401 uses a narrow lens: forward-only smoke scope.
...
## Minimal Example
Imagine a bounded run note for topic B04-RTS-0401.
The reviewer opens a a100_smoke_tests artifact and asks whether the primary signal for forward-only smoke scope is present.
...
## Review Notes
This file exists to teach one distinct review target: forward-only smoke scope.
