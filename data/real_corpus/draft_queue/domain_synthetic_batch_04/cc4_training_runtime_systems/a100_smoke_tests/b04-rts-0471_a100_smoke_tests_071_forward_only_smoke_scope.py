...
def build_a100_smoke_tests_0471(rows):
    summary = {
        'topic_id': 'B04-RTS-0471',
        'focus': 'forward-only smoke scope',
        'row_count': len(rows),
...
def check_a100_smoke_tests_70(summary):
    flags = {
        'has_rows': summary['row_count'] > 0,
        'range_known': 'min_value' in summary and 'max_value' in summary,
        'focus_matches': summary['focus'] == 'forward-only smoke scope',
    }
...
summary = build_a100_smoke_tests_0471(sample_rows)
flags = check_a100_smoke_tests_70(summary)
print(summary)
print(flags)

# Teaching note:
# This example demonstrates forward-only smoke scope.
