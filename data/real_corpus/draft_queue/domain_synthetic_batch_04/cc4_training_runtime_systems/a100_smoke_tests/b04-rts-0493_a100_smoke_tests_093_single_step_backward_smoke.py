...
def build_a100_smoke_tests_0493(rows):
    summary = {
        'topic_id': 'B04-RTS-0493',
        'focus': 'single-step backward smoke',
        'row_count': len(rows),
...
def check_a100_smoke_tests_92(summary):
    flags = {
        'has_rows': summary['row_count'] > 0,
        'range_known': 'min_value' in summary and 'max_value' in summary,
        'focus_matches': summary['focus'] == 'single-step backward smoke',
    }
    flags['review_ready'] = all(flags.values())
    return flags


sample_rows = [1.17, 1.42, 1.67, 1.92, 2.17]
summary = build_a100_smoke_tests_0493(sample_rows)
flags = check_a100_smoke_tests_92(summary)
print(summary)
print(flags)

# Teaching note:
# This example demonstrates single-step backward smoke.
