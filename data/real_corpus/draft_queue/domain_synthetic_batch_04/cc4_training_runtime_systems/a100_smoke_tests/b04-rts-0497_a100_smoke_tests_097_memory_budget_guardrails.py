...
def build_a100_smoke_tests_0497(rows):
    summary = {
        'topic_id': 'B04-RTS-0497',
        'focus': 'memory budget guardrails',
        'row_count': len(rows),
...
def check_a100_smoke_tests_96(summary):
    flags = {
        'has_rows': summary['row_count'] > 0,
        'range_known': 'min_value' in summary and 'max_value' in summary,
        'focus_matches': summary['focus'] == 'memory budget guardrails',
    }
...
summary = build_a100_smoke_tests_0497(sample_rows)
flags = check_a100_smoke_tests_96(summary)
print(summary)
print(flags)

# Teaching note:
# This example demonstrates memory budget guardrails.
