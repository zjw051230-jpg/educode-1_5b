...
def build_checkpointing_0071(rows):
    summary = {
        'topic_id': 'B04-RTS-0071',
        'focus': 'top-level checkpoint key checks',
        'row_count': len(rows),
...
def check_checkpointing_70(summary):
    flags = {
        'has_rows': summary['row_count'] > 0,
        'range_known': 'min_value' in summary and 'max_value' in summary,
        'focus_matches': summary['focus'] == 'top-level checkpoint key checks',
    }
    flags['review_ready'] = all(flags.values())
    return flags


sample_rows = [1.25, 1.5, 1.75, 2.0, 2.25]
summary = build_checkpointing_0071(sample_rows)
flags = check_checkpointing_70(summary)
print(summary)
print(flags)

# Teaching note:
# This example demonstrates top-level checkpoint key checks.
