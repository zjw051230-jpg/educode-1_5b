...
def build_checkpointing_0073(rows):
    summary = {
        'topic_id': 'B04-RTS-0073',
        'focus': 'reload shape verification',
        'row_count': len(rows),
...
def check_checkpointing_72(summary):
    flags = {
        'has_rows': summary['row_count'] > 0,
        'range_known': 'min_value' in summary and 'max_value' in summary,
        'focus_matches': summary['focus'] == 'reload shape verification',
    }
    flags['review_ready'] = all(flags.values())
    return flags


sample_rows = [1.47, 1.72, 1.97, 2.22, 2.47]
summary = build_checkpointing_0073(sample_rows)
flags = check_checkpointing_72(summary)
print(summary)
print(flags)

# Teaching note:
# This example demonstrates reload shape verification.
