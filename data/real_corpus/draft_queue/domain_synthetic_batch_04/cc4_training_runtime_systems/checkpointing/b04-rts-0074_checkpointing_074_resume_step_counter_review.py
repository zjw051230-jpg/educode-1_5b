...
def build_checkpointing_0074(rows):
    summary = {
        'topic_id': 'B04-RTS-0074',
        'focus': 'resume step counter review',
        'row_count': len(rows),
...
def check_checkpointing_73(summary):
    flags = {
        'has_rows': summary['row_count'] > 0,
        'range_known': 'min_value' in summary and 'max_value' in summary,
        'focus_matches': summary['focus'] == 'resume step counter review',
    }
    flags['review_ready'] = all(flags.values())
    return flags


sample_rows = [1.58, 1.83, 2.08, 2.33, 2.58]
summary = build_checkpointing_0074(sample_rows)
flags = check_checkpointing_73(summary)
print(summary)
print(flags)

# Teaching note:
# This example demonstrates resume step counter review.
