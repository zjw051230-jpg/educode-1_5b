...
def build_checkpointing_0077(rows):
    summary = {
        'topic_id': 'B04-RTS-0077',
        'focus': 'best-vs-latest artifact meaning',
        'row_count': len(rows),
...
def check_checkpointing_76(summary):
    flags = {
        'has_rows': summary['row_count'] > 0,
        'range_known': 'min_value' in summary and 'max_value' in summary,
        'focus_matches': summary['focus'] == 'best-vs-latest artifact meaning',
    }
    flags['review_ready'] = all(flags.values())
    return flags


sample_rows = [1.91, 2.16, 2.41, 2.66, 2.91]
summary = build_checkpointing_0077(sample_rows)
flags = check_checkpointing_76(summary)
print(summary)
print(flags)

# Teaching note:
# This example demonstrates best-vs-latest artifact meaning.
