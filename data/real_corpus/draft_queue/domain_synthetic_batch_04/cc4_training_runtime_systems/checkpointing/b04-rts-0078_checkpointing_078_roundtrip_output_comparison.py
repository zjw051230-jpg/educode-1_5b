...
def build_checkpointing_0078(rows):
    summary = {
        'topic_id': 'B04-RTS-0078',
        'focus': 'roundtrip output comparison',
        'row_count': len(rows),
...
def check_checkpointing_77(summary):
    flags = {
        'has_rows': summary['row_count'] > 0,
        'range_known': 'min_value' in summary and 'max_value' in summary,
        'focus_matches': summary['focus'] == 'roundtrip output comparison',
    }
    flags['review_ready'] = all(flags.values())
    return flags


sample_rows = [2.02, 2.27, 2.52, 2.77, 3.02]
summary = build_checkpointing_0078(sample_rows)
flags = check_checkpointing_77(summary)
print(summary)
print(flags)

# Teaching note:
# This example demonstrates roundtrip output comparison.
