...
def build_checkpointing_0090(rows):
    summary = {
        'topic_id': 'B04-RTS-0090',
        'focus': 'checkpoint directory naming discipline',
        'row_count': len(rows),
...
def check_checkpointing_89(summary):
    flags = {
        'has_rows': summary['row_count'] > 0,
        'range_known': 'min_value' in summary and 'max_value' in summary,
        'focus_matches': summary['focus'] == 'checkpoint directory naming discipline',
    }
    flags['review_ready'] = all(flags.values())
    return flags


sample_rows = [1.64, 1.89, 2.14, 2.39, 2.64]
summary = build_checkpointing_0090(sample_rows)
flags = check_checkpointing_89(summary)
print(summary)
print(flags)

# Teaching note:
# This example demonstrates checkpoint directory naming discipline.
