...
def build_checkpointing_0096(rows):
    summary = {
        'topic_id': 'B04-RTS-0096',
        'focus': 'checkpoint metadata design',
        'row_count': len(rows),
...
def check_checkpointing_95(summary):
    flags = {
        'has_rows': summary['row_count'] > 0,
        'range_known': 'min_value' in summary and 'max_value' in summary,
        'focus_matches': summary['focus'] == 'checkpoint metadata design',
    }
...
summary = build_checkpointing_0096(sample_rows)
flags = check_checkpointing_95(summary)
print(summary)
print(flags)

# Teaching note:
# This example demonstrates checkpoint metadata design.
