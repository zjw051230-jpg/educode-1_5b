...
def build_cuda_memory_0376(rows):
    summary = {
        'topic_id': 'B04-RTS-0376',
        'focus': 'fragmentation suspicion notes',
        'row_count': len(rows),
...
def check_cuda_memory_75(summary):
    flags = {
        'has_rows': summary['row_count'] > 0,
        'range_known': 'min_value' in summary and 'max_value' in summary,
        'focus_matches': summary['focus'] == 'fragmentation suspicion notes',
    }
    flags['review_ready'] = all(flags.values())
    return flags


sample_rows = [1.2, 1.45, 1.7, 1.95, 2.2]
summary = build_cuda_memory_0376(sample_rows)
flags = check_cuda_memory_75(summary)
print(summary)
print(flags)

# Teaching note:
# This example demonstrates fragmentation suspicion notes.
