...
def build_cuda_memory_0398(rows):
    summary = {
        'topic_id': 'B04-RTS-0398',
        'focus': 'host-device transfer caveats',
        'row_count': len(rows),
...
def check_cuda_memory_97(summary):
    flags = {
        'has_rows': summary['row_count'] > 0,
        'range_known': 'min_value' in summary and 'max_value' in summary,
        'focus_matches': summary['focus'] == 'host-device transfer caveats',
    }
...
summary = build_cuda_memory_0398(sample_rows)
flags = check_cuda_memory_97(summary)
print(summary)
print(flags)

# Teaching note:
# This example demonstrates host-device transfer caveats.
