...
def build_experiment_logging_0578(rows):
    summary = {
        'topic_id': 'B04-RTS-0578',
        'focus': 'artifact naming consistency',
        'row_count': len(rows),
...
def check_experiment_logging_77(summary):
    flags = {
        'has_rows': summary['row_count'] > 0,
        'range_known': 'min_value' in summary and 'max_value' in summary,
        'focus_matches': summary['focus'] == 'artifact naming consistency',
    }
    flags['review_ready'] = all(flags.values())
    return flags


sample_rows = [1.02, 1.27, 1.52, 1.77, 2.02]
summary = build_experiment_logging_0578(sample_rows)
flags = check_experiment_logging_77(summary)
print(summary)
print(flags)

# Teaching note:
# This example demonstrates artifact naming consistency.
