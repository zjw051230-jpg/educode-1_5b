...
def build_experiment_logging_0591(rows):
    summary = {
        'topic_id': 'B04-RTS-0591',
        'focus': 'metrics row event typing',
        'row_count': len(rows),
...
def check_experiment_logging_90(summary):
    flags = {
        'has_rows': summary['row_count'] > 0,
        'range_known': 'min_value' in summary and 'max_value' in summary,
        'focus_matches': summary['focus'] == 'metrics row event typing',
    }
...
summary = build_experiment_logging_0591(sample_rows)
flags = check_experiment_logging_90(summary)
print(summary)
print(flags)

# Teaching note:
# This example demonstrates metrics row event typing.
