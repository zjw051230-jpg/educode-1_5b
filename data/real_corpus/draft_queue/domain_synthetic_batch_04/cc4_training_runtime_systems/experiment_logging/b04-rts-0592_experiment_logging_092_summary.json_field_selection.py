...
def build_experiment_logging_0592(rows):
    summary = {
        'topic_id': 'B04-RTS-0592',
        'focus': 'summary.json field selection',
        'row_count': len(rows),
...
def check_experiment_logging_91(summary):
    flags = {
        'has_rows': summary['row_count'] > 0,
        'range_known': 'min_value' in summary and 'max_value' in summary,
        'focus_matches': summary['focus'] == 'summary.json field selection',
    }
    flags['review_ready'] = all(flags.values())
    return flags


sample_rows = [2.56, 2.81, 3.06, 3.31, 3.56]
summary = build_experiment_logging_0592(sample_rows)
flags = check_experiment_logging_91(summary)
print(summary)
print(flags)

# Teaching note:
# This example demonstrates summary.json field selection.
