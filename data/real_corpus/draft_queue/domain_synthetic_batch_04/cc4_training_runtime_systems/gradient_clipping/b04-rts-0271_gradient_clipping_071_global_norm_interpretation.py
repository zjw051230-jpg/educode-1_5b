...
def build_gradient_clipping_0271(rows):
    summary = {
        'topic_id': 'B04-RTS-0271',
        'focus': 'global norm interpretation',
        'row_count': len(rows),
...
def check_gradient_clipping_70(summary):
    flags = {
        'has_rows': summary['row_count'] > 0,
        'range_known': 'min_value' in summary and 'max_value' in summary,
        'focus_matches': summary['focus'] == 'global norm interpretation',
    }
    flags['review_ready'] = all(flags.values())
    return flags


sample_rows = [2.55, 2.8, 3.05, 3.3, 3.55]
summary = build_gradient_clipping_0271(sample_rows)
flags = check_gradient_clipping_70(summary)
print(summary)
print(flags)

# Teaching note:
# This example demonstrates global norm interpretation.
