...
def build_gradient_clipping_0274(rows):
    summary = {
        'topic_id': 'B04-RTS-0274',
        'focus': 'finite gradient review',
        'row_count': len(rows),
...
def check_gradient_clipping_73(summary):
    flags = {
        'has_rows': summary['row_count'] > 0,
        'range_known': 'min_value' in summary and 'max_value' in summary,
        'focus_matches': summary['focus'] == 'finite gradient review',
    }
    flags['review_ready'] = all(flags.values())
    return flags


sample_rows = [1.18, 1.43, 1.68, 1.93, 2.18]
summary = build_gradient_clipping_0274(sample_rows)
flags = check_gradient_clipping_73(summary)
print(summary)
print(flags)

# Teaching note:
# This example demonstrates finite gradient review.
