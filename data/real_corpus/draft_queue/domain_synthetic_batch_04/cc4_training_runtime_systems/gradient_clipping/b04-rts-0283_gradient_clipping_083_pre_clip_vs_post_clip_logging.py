...
def build_gradient_clipping_0283(rows):
    summary = {
        'topic_id': 'B04-RTS-0283',
        'focus': 'pre-clip vs post-clip logging',
        'row_count': len(rows),
...
def check_gradient_clipping_82(summary):
    flags = {
        'has_rows': summary['row_count'] > 0,
        'range_known': 'min_value' in summary and 'max_value' in summary,
        'focus_matches': summary['focus'] == 'pre-clip vs post-clip logging',
    }
    flags['review_ready'] = all(flags.values())
    return flags


sample_rows = [2.17, 2.42, 2.67, 2.92, 3.17]
summary = build_gradient_clipping_0283(sample_rows)
flags = check_gradient_clipping_82(summary)
print(summary)
print(flags)

# Teaching note:
# This example demonstrates pre-clip vs post-clip logging.
