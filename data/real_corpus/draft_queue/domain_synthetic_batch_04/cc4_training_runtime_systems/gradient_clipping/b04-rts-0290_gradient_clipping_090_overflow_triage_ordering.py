...
def build_gradient_clipping_0290(rows):
    summary = {
        'topic_id': 'B04-RTS-0290',
        'focus': 'overflow triage ordering',
        'row_count': len(rows),
...
def check_gradient_clipping_89(summary):
    flags = {
        'has_rows': summary['row_count'] > 0,
        'range_known': 'min_value' in summary and 'max_value' in summary,
        'focus_matches': summary['focus'] == 'overflow triage ordering',
    }
    flags['review_ready'] = all(flags.values())
    return flags


sample_rows = [1.24, 1.49, 1.74, 1.99, 2.24]
summary = build_gradient_clipping_0290(sample_rows)
flags = check_gradient_clipping_89(summary)
print(summary)
print(flags)

# Teaching note:
# This example demonstrates overflow triage ordering.
