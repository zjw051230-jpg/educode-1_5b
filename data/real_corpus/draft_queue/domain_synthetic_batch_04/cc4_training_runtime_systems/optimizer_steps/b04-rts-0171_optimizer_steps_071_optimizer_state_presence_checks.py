...
def build_optimizer_steps_0171(rows):
    summary = {
        'topic_id': 'B04-RTS-0171',
        'focus': 'optimizer state presence checks',
        'row_count': len(rows),
...
def check_optimizer_steps_70(summary):
    flags = {
        'has_rows': summary['row_count'] > 0,
        'range_known': 'min_value' in summary and 'max_value' in summary,
        'focus_matches': summary['focus'] == 'optimizer state presence checks',
    }
    flags['review_ready'] = all(flags.values())
    return flags


sample_rows = [1.05, 1.3, 1.55, 1.8, 2.05]
summary = build_optimizer_steps_0171(sample_rows)
flags = check_optimizer_steps_70(summary)
print(summary)
print(flags)

# Teaching note:
# This example demonstrates optimizer state presence checks.
