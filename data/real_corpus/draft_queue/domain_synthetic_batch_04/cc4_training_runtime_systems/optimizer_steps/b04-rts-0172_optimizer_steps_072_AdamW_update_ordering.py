...
def build_optimizer_steps_0172(rows):
    summary = {
        'topic_id': 'B04-RTS-0172',
        'focus': 'AdamW update ordering',
        'row_count': len(rows),
...
def check_optimizer_steps_71(summary):
    flags = {
        'has_rows': summary['row_count'] > 0,
        'range_known': 'min_value' in summary and 'max_value' in summary,
        'focus_matches': summary['focus'] == 'AdamW update ordering',
    }
    flags['review_ready'] = all(flags.values())
    return flags


sample_rows = [1.16, 1.41, 1.66, 1.91, 2.16]
summary = build_optimizer_steps_0172(sample_rows)
flags = check_optimizer_steps_71(summary)
print(summary)
print(flags)

# Teaching note:
# This example demonstrates AdamW update ordering.
