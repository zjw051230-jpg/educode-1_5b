...
def build_optimizer_steps_0175(rows):
    summary = {
        'topic_id': 'B04-RTS-0175',
        'focus': 'param-group summary checks',
        'row_count': len(rows),
...
def check_optimizer_steps_74(summary):
    flags = {
        'has_rows': summary['row_count'] > 0,
        'range_known': 'min_value' in summary and 'max_value' in summary,
        'focus_matches': summary['focus'] == 'param-group summary checks',
    }
    flags['review_ready'] = all(flags.values())
    return flags


sample_rows = [1.49, 1.74, 1.99, 2.24, 2.49]
summary = build_optimizer_steps_0175(sample_rows)
flags = check_optimizer_steps_74(summary)
print(summary)
print(flags)

# Teaching note:
# This example demonstrates param-group summary checks.
