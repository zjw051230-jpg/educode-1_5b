...
def build_optimizer_steps_0187(rows):
    summary = {
        'topic_id': 'B04-RTS-0187',
        'focus': 'weight-decay separation notes',
        'row_count': len(rows),
...
def check_optimizer_steps_86(summary):
    flags = {
        'has_rows': summary['row_count'] > 0,
        'range_known': 'min_value' in summary and 'max_value' in summary,
        'focus_matches': summary['focus'] == 'weight-decay separation notes',
    }
    flags['review_ready'] = all(flags.values())
    return flags


sample_rows = [1.11, 1.36, 1.61, 1.86, 2.11]
summary = build_optimizer_steps_0187(sample_rows)
flags = check_optimizer_steps_86(summary)
print(summary)
print(flags)

# Teaching note:
# This example demonstrates weight-decay separation notes.
