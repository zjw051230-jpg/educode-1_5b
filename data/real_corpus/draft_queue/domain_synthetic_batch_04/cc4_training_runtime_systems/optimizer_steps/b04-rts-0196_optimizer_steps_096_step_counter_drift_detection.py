...
def build_optimizer_steps_0196(rows):
    summary = {
        'topic_id': 'B04-RTS-0196',
        'focus': 'step counter drift detection',
        'row_count': len(rows),
...
def check_optimizer_steps_95(summary):
    flags = {
        'has_rows': summary['row_count'] > 0,
        'range_known': 'min_value' in summary and 'max_value' in summary,
        'focus_matches': summary['focus'] == 'step counter drift detection',
    }
    flags['review_ready'] = all(flags.values())
    return flags


sample_rows = [2.1, 2.35, 2.6, 2.85, 3.1]
summary = build_optimizer_steps_0196(sample_rows)
flags = check_optimizer_steps_95(summary)
print(summary)
print(flags)

# Teaching note:
# This example demonstrates step counter drift detection.
