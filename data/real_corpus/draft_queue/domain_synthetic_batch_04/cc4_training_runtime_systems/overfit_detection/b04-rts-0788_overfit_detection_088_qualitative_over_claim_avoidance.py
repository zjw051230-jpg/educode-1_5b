...
def build_overfit_detection_0788(rows):
    summary = {
        'topic_id': 'B04-RTS-0788',
        'focus': 'qualitative over-claim avoidance',
        'row_count': len(rows),
...
def check_overfit_detection_87(summary):
    flags = {
        'has_rows': summary['row_count'] > 0,
        'range_known': 'min_value' in summary and 'max_value' in summary,
        'focus_matches': summary['focus'] == 'qualitative over-claim avoidance',
    }
    flags['review_ready'] = all(flags.values())
    return flags


sample_rows = [1.72, 1.97, 2.22, 2.47, 2.72]
summary = build_overfit_detection_0788(sample_rows)
flags = check_overfit_detection_87(summary)
print(summary)
print(flags)

# Teaching note:
# This example demonstrates qualitative over-claim avoidance.
