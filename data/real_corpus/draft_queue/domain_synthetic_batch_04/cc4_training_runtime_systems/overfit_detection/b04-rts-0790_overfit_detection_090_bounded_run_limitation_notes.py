...
def build_overfit_detection_0790(rows):
    summary = {
        'topic_id': 'B04-RTS-0790',
        'focus': 'bounded-run limitation notes',
        'row_count': len(rows),
...
def check_overfit_detection_89(summary):
    flags = {
        'has_rows': summary['row_count'] > 0,
        'range_known': 'min_value' in summary and 'max_value' in summary,
        'focus_matches': summary['focus'] == 'bounded-run limitation notes',
    }
    flags['review_ready'] = all(flags.values())
    return flags


sample_rows = [1.94, 2.19, 2.44, 2.69, 2.94]
summary = build_overfit_detection_0790(sample_rows)
flags = check_overfit_detection_89(summary)
print(summary)
print(flags)

# Teaching note:
# This example demonstrates bounded-run limitation notes.
