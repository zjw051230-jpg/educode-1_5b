...
def build_overfit_detection_0791(rows):
    summary = {
        'topic_id': 'B04-RTS-0791',
        'focus': 'train-vs-val drift notes',
        'row_count': len(rows),
...
def check_overfit_detection_90(summary):
    flags = {
        'has_rows': summary['row_count'] > 0,
        'range_known': 'min_value' in summary and 'max_value' in summary,
        'focus_matches': summary['focus'] == 'train-vs-val drift notes',
    }
    flags['review_ready'] = all(flags.values())
    return flags


sample_rows = [2.05, 2.3, 2.55, 2.8, 3.05]
summary = build_overfit_detection_0791(sample_rows)
flags = check_overfit_detection_90(summary)
print(summary)
print(flags)

# Teaching note:
# This example demonstrates train-vs-val drift notes.
