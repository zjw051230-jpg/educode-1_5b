...
def build_overfit_detection_0792(rows):
    summary = {
        'topic_id': 'B04-RTS-0792',
        'focus': 'best-step tracking',
        'row_count': len(rows),
...
def check_overfit_detection_91(summary):
    flags = {
        'has_rows': summary['row_count'] > 0,
        'range_known': 'min_value' in summary and 'max_value' in summary,
        'focus_matches': summary['focus'] == 'best-step tracking',
    }
    flags['review_ready'] = all(flags.values())
    return flags


sample_rows = [2.16, 2.41, 2.66, 2.91, 3.16]
summary = build_overfit_detection_0792(sample_rows)
flags = check_overfit_detection_91(summary)
print(summary)
print(flags)

# Teaching note:
# This example demonstrates best-step tracking.
