...
def build_overfit_detection_0795(rows):
    summary = {
        'topic_id': 'B04-RTS-0795',
        'focus': 'plateau vs regression review',
        'row_count': len(rows),
...
def check_overfit_detection_94(summary):
    flags = {
        'has_rows': summary['row_count'] > 0,
        'range_known': 'min_value' in summary and 'max_value' in summary,
        'focus_matches': summary['focus'] == 'plateau vs regression review',
    }
    flags['review_ready'] = all(flags.values())
    return flags


sample_rows = [2.49, 2.74, 2.99, 3.24, 3.49]
summary = build_overfit_detection_0795(sample_rows)
flags = check_overfit_detection_94(summary)
print(summary)
print(flags)

# Teaching note:
# This example demonstrates plateau vs regression review.
