...
def build_overfit_detection_0797(rows):
    summary = {
        'topic_id': 'B04-RTS-0797',
        'focus': 'loss window interpretation',
        'row_count': len(rows),
...
def check_overfit_detection_96(summary):
    flags = {
        'has_rows': summary['row_count'] > 0,
        'range_known': 'min_value' in summary and 'max_value' in summary,
        'focus_matches': summary['focus'] == 'loss window interpretation',
    }
    flags['review_ready'] = all(flags.values())
    return flags


sample_rows = [2.71, 2.96, 3.21, 3.46, 3.71]
summary = build_overfit_detection_0797(sample_rows)
flags = check_overfit_detection_96(summary)
print(summary)
print(flags)

# Teaching note:
# This example demonstrates loss window interpretation.
