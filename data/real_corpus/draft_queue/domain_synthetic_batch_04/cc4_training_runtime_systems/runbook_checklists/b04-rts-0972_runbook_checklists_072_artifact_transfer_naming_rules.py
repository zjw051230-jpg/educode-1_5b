...
def build_runbook_checklists_0972(rows):
    summary = {
        'topic_id': 'B04-RTS-0972',
        'focus': 'artifact transfer naming rules',
        'row_count': len(rows),
...
def check_runbook_checklists_71(summary):
    flags = {
        'has_rows': summary['row_count'] > 0,
        'range_known': 'min_value' in summary and 'max_value' in summary,
        'focus_matches': summary['focus'] == 'artifact transfer naming rules',
    }
    flags['review_ready'] = all(flags.values())
    return flags


sample_rows = [1.26, 1.51, 1.76, 2.01, 2.26]
summary = build_runbook_checklists_0972(sample_rows)
flags = check_runbook_checklists_71(summary)
print(summary)
print(flags)

# Teaching note:
# This example demonstrates artifact transfer naming rules.
