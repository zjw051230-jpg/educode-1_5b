...
def build_runbook_checklists_0973(rows):
    summary = {
        'topic_id': 'B04-RTS-0973',
        'focus': 'handoff summary checklist',
        'row_count': len(rows),
...
def check_runbook_checklists_72(summary):
    flags = {
        'has_rows': summary['row_count'] > 0,
        'range_known': 'min_value' in summary and 'max_value' in summary,
        'focus_matches': summary['focus'] == 'handoff summary checklist',
    }
    flags['review_ready'] = all(flags.values())
    return flags


sample_rows = [1.37, 1.62, 1.87, 2.12, 2.37]
summary = build_runbook_checklists_0973(sample_rows)
flags = check_runbook_checklists_72(summary)
print(summary)
print(flags)

# Teaching note:
# This example demonstrates handoff summary checklist.
