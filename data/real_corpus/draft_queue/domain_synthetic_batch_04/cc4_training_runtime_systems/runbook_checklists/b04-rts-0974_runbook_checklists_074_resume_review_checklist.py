...
def build_runbook_checklists_0974(rows):
    summary = {
        'topic_id': 'B04-RTS-0974',
        'focus': 'resume review checklist',
        'row_count': len(rows),
...
def check_runbook_checklists_73(summary):
    flags = {
        'has_rows': summary['row_count'] > 0,
        'range_known': 'min_value' in summary and 'max_value' in summary,
        'focus_matches': summary['focus'] == 'resume review checklist',
    }
    flags['review_ready'] = all(flags.values())
    return flags


sample_rows = [1.48, 1.73, 1.98, 2.23, 2.48]
summary = build_runbook_checklists_0974(sample_rows)
flags = check_runbook_checklists_73(summary)
print(summary)
print(flags)

# Teaching note:
# This example demonstrates resume review checklist.
