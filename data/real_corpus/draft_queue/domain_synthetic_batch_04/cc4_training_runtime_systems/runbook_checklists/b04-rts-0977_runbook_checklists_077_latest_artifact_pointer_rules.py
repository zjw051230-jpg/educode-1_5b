...
def build_runbook_checklists_0977(rows):
    summary = {
        'topic_id': 'B04-RTS-0977',
        'focus': 'latest artifact pointer rules',
        'row_count': len(rows),
...
def check_runbook_checklists_76(summary):
    flags = {
        'has_rows': summary['row_count'] > 0,
        'range_known': 'min_value' in summary and 'max_value' in summary,
        'focus_matches': summary['focus'] == 'latest artifact pointer rules',
    }
    flags['review_ready'] = all(flags.values())
    return flags


sample_rows = [1.81, 2.06, 2.31, 2.56, 2.81]
summary = build_runbook_checklists_0977(sample_rows)
flags = check_runbook_checklists_76(summary)
print(summary)
print(flags)

# Teaching note:
# This example demonstrates latest artifact pointer rules.
