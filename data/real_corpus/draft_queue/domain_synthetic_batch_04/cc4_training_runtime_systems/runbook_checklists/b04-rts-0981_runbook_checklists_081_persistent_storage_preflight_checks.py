...
def build_runbook_checklists_0981(rows):
    summary = {
        'topic_id': 'B04-RTS-0981',
        'focus': 'persistent storage preflight checks',
        'row_count': len(rows),
...
def check_runbook_checklists_80(summary):
    flags = {
        'has_rows': summary['row_count'] > 0,
        'range_known': 'min_value' in summary and 'max_value' in summary,
        'focus_matches': summary['focus'] == 'persistent storage preflight checks',
    }
    flags['review_ready'] = all(flags.values())
    return flags


sample_rows = [2.25, 2.5, 2.75, 3.0, 3.25]
summary = build_runbook_checklists_0981(sample_rows)
flags = check_runbook_checklists_80(summary)
print(summary)
print(flags)

# Teaching note:
# This example demonstrates persistent storage preflight checks.
