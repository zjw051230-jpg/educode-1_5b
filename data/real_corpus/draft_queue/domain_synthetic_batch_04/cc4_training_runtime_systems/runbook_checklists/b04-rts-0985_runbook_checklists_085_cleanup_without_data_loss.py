...
def build_runbook_checklists_0985(rows):
    summary = {
        'topic_id': 'B04-RTS-0985',
        'focus': 'cleanup without data loss',
        'row_count': len(rows),
...
def check_runbook_checklists_84(summary):
    flags = {
        'has_rows': summary['row_count'] > 0,
        'range_known': 'min_value' in summary and 'max_value' in summary,
        'focus_matches': summary['focus'] == 'cleanup without data loss',
    }
    flags['review_ready'] = all(flags.values())
    return flags


sample_rows = [2.69, 2.94, 3.19, 3.44, 3.69]
summary = build_runbook_checklists_0985(sample_rows)
flags = check_runbook_checklists_84(summary)
print(summary)
print(flags)

# Teaching note:
# This example demonstrates cleanup without data loss.
