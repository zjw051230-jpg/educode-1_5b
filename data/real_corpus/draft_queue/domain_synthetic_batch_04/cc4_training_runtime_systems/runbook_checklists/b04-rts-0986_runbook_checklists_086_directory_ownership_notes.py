...
def build_runbook_checklists_0986(rows):
    summary = {
        'topic_id': 'B04-RTS-0986',
        'focus': 'directory ownership notes',
        'row_count': len(rows),
...
def check_runbook_checklists_85(summary):
    flags = {
        'has_rows': summary['row_count'] > 0,
        'range_known': 'min_value' in summary and 'max_value' in summary,
        'focus_matches': summary['focus'] == 'directory ownership notes',
    }
    flags['review_ready'] = all(flags.values())
    return flags


sample_rows = [1.1, 1.35, 1.6, 1.85, 2.1]
summary = build_runbook_checklists_0986(sample_rows)
flags = check_runbook_checklists_85(summary)
print(summary)
print(flags)

# Teaching note:
# This example demonstrates directory ownership notes.
