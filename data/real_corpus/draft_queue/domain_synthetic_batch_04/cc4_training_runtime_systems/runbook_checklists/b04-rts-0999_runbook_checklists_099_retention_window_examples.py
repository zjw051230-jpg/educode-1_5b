...
def build_runbook_checklists_0999(rows):
    summary = {
        'topic_id': 'B04-RTS-0999',
        'focus': 'retention window examples',
        'row_count': len(rows),
...
def check_runbook_checklists_98(summary):
    flags = {
        'has_rows': summary['row_count'] > 0,
        'range_known': 'min_value' in summary and 'max_value' in summary,
        'focus_matches': summary['focus'] == 'retention window examples',
    }
    flags['review_ready'] = all(flags.values())
    return flags


sample_rows = [2.53, 2.78, 3.03, 3.28, 3.53]
summary = build_runbook_checklists_0999(sample_rows)
flags = check_runbook_checklists_98(summary)
print(summary)
print(flags)

# Teaching note:
# This example demonstrates retention window examples.
