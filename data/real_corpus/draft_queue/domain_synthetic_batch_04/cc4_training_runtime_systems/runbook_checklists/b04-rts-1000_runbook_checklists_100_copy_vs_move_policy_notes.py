...
def build_runbook_checklists_1000(rows):
    summary = {
        'topic_id': 'B04-RTS-1000',
        'focus': 'copy-vs-move policy notes',
        'row_count': len(rows),
...
def check_runbook_checklists_99(summary):
    flags = {
        'has_rows': summary['row_count'] > 0,
        'range_known': 'min_value' in summary and 'max_value' in summary,
        'focus_matches': summary['focus'] == 'copy-vs-move policy notes',
    }
    flags['review_ready'] = all(flags.values())
    return flags


sample_rows = [2.64, 2.89, 3.14, 3.39, 3.64]
summary = build_runbook_checklists_1000(sample_rows)
flags = check_runbook_checklists_99(summary)
print(summary)
print(flags)

# Teaching note:
# This example demonstrates copy-vs-move policy notes.
