...
# Tokens-Per-Second Framing Review Notes

## Concept
Tokens-per-second framing is the main teaching target for this draft candidate.
...
## Explanation
Example index 851 uses a narrow lens: tokens-per-second framing.
...
## Minimal Example
Imagine a bounded run note for topic B04-RTS-0851.
The reviewer opens a throughput_profiles artifact and asks whether the primary signal for tokens-per-second framing is present.
...
## Review Notes
This file exists to teach one distinct review target: tokens-per-second framing.
