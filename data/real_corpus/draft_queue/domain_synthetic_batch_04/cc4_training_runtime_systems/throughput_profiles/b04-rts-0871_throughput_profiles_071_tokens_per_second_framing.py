...
def build_throughput_profiles_0871(rows):
    summary = {
        'topic_id': 'B04-RTS-0871',
        'focus': 'tokens-per-second framing',
        'row_count': len(rows),
...
def check_throughput_profiles_70(summary):
    flags = {
        'has_rows': summary['row_count'] > 0,
        'range_known': 'min_value' in summary and 'max_value' in summary,
        'focus_matches': summary['focus'] == 'tokens-per-second framing',
    }
    flags['review_ready'] = all(flags.values())
    return flags


sample_rows = [1.35, 1.6, 1.85, 2.1, 2.35]
summary = build_throughput_profiles_0871(sample_rows)
flags = check_throughput_profiles_70(summary)
print(summary)
print(flags)

# Teaching note:
# This example demonstrates tokens-per-second framing.
