...
def build_throughput_profiles_0874(rows):
    summary = {
        'topic_id': 'B04-RTS-0874',
        'focus': 'seq-len profile comparison',
        'row_count': len(rows),
...
def check_throughput_profiles_73(summary):
    flags = {
        'has_rows': summary['row_count'] > 0,
        'range_known': 'min_value' in summary and 'max_value' in summary,
        'focus_matches': summary['focus'] == 'seq-len profile comparison',
    }
    flags['review_ready'] = all(flags.values())
    return flags


sample_rows = [1.68, 1.93, 2.18, 2.43, 2.68]
summary = build_throughput_profiles_0874(sample_rows)
flags = check_throughput_profiles_73(summary)
print(summary)
print(flags)

# Teaching note:
# This example demonstrates seq-len profile comparison.
