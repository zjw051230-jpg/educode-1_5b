...
def build_throughput_profiles_0878(rows):
    summary = {
        'topic_id': 'B04-RTS-0878',
        'focus': 'load-vs-compute interpretation',
        'row_count': len(rows),
...
def check_throughput_profiles_77(summary):
    flags = {
        'has_rows': summary['row_count'] > 0,
        'range_known': 'min_value' in summary and 'max_value' in summary,
        'focus_matches': summary['focus'] == 'load-vs-compute interpretation',
    }
    flags['review_ready'] = all(flags.values())
    return flags


sample_rows = [2.12, 2.37, 2.62, 2.87, 3.12]
summary = build_throughput_profiles_0878(sample_rows)
flags = check_throughput_profiles_77(summary)
print(summary)
print(flags)

# Teaching note:
# This example demonstrates load-vs-compute interpretation.
