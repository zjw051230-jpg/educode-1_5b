...
def build_throughput_profiles_0880(rows):
    summary = {
        'topic_id': 'B04-RTS-0880',
        'focus': 'regression report phrasing',
        'row_count': len(rows),
...
def check_throughput_profiles_79(summary):
    flags = {
        'has_rows': summary['row_count'] > 0,
        'range_known': 'min_value' in summary and 'max_value' in summary,
        'focus_matches': summary['focus'] == 'regression report phrasing',
    }
    flags['review_ready'] = all(flags.values())
    return flags


sample_rows = [2.34, 2.59, 2.84, 3.09, 3.34]
summary = build_throughput_profiles_0880(sample_rows)
flags = check_throughput_profiles_79(summary)
print(summary)
print(flags)

# Teaching note:
# This example demonstrates regression report phrasing.
