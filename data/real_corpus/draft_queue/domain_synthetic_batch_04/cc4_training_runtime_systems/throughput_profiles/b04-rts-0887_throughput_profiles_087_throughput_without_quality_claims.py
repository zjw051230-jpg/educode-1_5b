...
def build_throughput_profiles_0887(rows):
    summary = {
        'topic_id': 'B04-RTS-0887',
        'focus': 'throughput without quality claims',
        'row_count': len(rows),
...
def check_throughput_profiles_86(summary):
    flags = {
        'has_rows': summary['row_count'] > 0,
        'range_known': 'min_value' in summary and 'max_value' in summary,
        'focus_matches': summary['focus'] == 'throughput without quality claims',
    }
    flags['review_ready'] = all(flags.values())
    return flags


sample_rows = [1.41, 1.66, 1.91, 2.16, 2.41]
summary = build_throughput_profiles_0887(sample_rows)
flags = check_throughput_profiles_86(summary)
print(summary)
print(flags)

# Teaching note:
# This example demonstrates throughput without quality claims.
