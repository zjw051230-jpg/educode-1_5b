...
def build_throughput_profiles_0889(rows):
    summary = {
        'topic_id': 'B04-RTS-0889',
        'focus': 'latency percentile summaries',
        'row_count': len(rows),
...
def check_throughput_profiles_88(summary):
    flags = {
        'has_rows': summary['row_count'] > 0,
        'range_known': 'min_value' in summary and 'max_value' in summary,
        'focus_matches': summary['focus'] == 'latency percentile summaries',
    }
    flags['review_ready'] = all(flags.values())
    return flags


sample_rows = [1.63, 1.88, 2.13, 2.38, 2.63]
summary = build_throughput_profiles_0889(sample_rows)
flags = check_throughput_profiles_88(summary)
print(summary)
print(flags)

# Teaching note:
# This example demonstrates latency percentile summaries.
