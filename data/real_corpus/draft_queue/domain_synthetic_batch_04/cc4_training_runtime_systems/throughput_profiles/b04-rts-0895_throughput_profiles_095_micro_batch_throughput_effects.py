...
def build_throughput_profiles_0895(rows):
    summary = {
        'topic_id': 'B04-RTS-0895',
        'focus': 'micro-batch throughput effects',
        'row_count': len(rows),
...
def check_throughput_profiles_94(summary):
    flags = {
        'has_rows': summary['row_count'] > 0,
        'range_known': 'min_value' in summary and 'max_value' in summary,
        'focus_matches': summary['focus'] == 'micro-batch throughput effects',
    }
    flags['review_ready'] = all(flags.values())
    return flags


sample_rows = [2.29, 2.54, 2.79, 3.04, 3.29]
summary = build_throughput_profiles_0895(sample_rows)
flags = check_throughput_profiles_94(summary)
print(summary)
print(flags)

# Teaching note:
# This example demonstrates micro-batch throughput effects.
