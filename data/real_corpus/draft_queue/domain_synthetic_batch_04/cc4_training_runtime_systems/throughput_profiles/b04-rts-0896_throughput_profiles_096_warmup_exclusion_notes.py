...
def build_throughput_profiles_0896(rows):
    summary = {
        'topic_id': 'B04-RTS-0896',
        'focus': 'warmup exclusion notes',
        'row_count': len(rows),
...
def check_throughput_profiles_95(summary):
    flags = {
        'has_rows': summary['row_count'] > 0,
        'range_known': 'min_value' in summary and 'max_value' in summary,
        'focus_matches': summary['focus'] == 'warmup exclusion notes',
    }
    flags['review_ready'] = all(flags.values())
    return flags


sample_rows = [2.4, 2.65, 2.9, 3.15, 3.4]
summary = build_throughput_profiles_0896(sample_rows)
flags = check_throughput_profiles_95(summary)
print(summary)
print(flags)

# Teaching note:
# This example demonstrates warmup exclusion notes.
