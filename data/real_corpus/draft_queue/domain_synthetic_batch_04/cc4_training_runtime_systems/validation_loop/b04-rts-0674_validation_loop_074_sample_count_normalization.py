...
def build_validation_loop_0674(rows):
    summary = {
        'topic_id': 'B04-RTS-0674',
        'focus': 'sample-count normalization',
        'row_count': len(rows),
...
def check_validation_loop_73(summary):
    flags = {
        'has_rows': summary['row_count'] > 0,
        'range_known': 'min_value' in summary and 'max_value' in summary,
        'focus_matches': summary['focus'] == 'sample-count normalization',
    }
    flags['review_ready'] = all(flags.values())
    return flags


sample_rows = [2.08, 2.33, 2.58, 2.83, 3.08]
summary = build_validation_loop_0674(sample_rows)
flags = check_validation_loop_73(summary)
print(summary)
print(flags)

# Teaching note:
# This example demonstrates sample-count normalization.
