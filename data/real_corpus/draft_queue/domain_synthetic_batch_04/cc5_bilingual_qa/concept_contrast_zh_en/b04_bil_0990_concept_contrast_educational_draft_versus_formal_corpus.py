...
"""Concept Contrasts and Evaluation Caution 90: Educational Draft Versus Formal Corpus

ZH: 教育型 draft 和正式训练语料的边界是什么。
...
def build_demo_rows() -> List[Dict[str, object]]:
    ...
    return [
        {
            "topic_id": "B04-BIL-0990",
            "group_theme": "Concept Contrasts and Evaluation Caution",
            "variant": "educational draft versus formal corpus",
...
# - 这个 Python 片段围绕 educational draft versus formal corpus 构造最小双语检查样例。
# - This Python snippet builds a tiny bilingual inspection example around educational draft versus formal corpus.
# - 它不访问网络、不读写真实用户数据，也不运行训练。
# - It does not touch networks, real user data, or training execution.
# - 第 90 个局部样例强调 reviewer 应先看结构，再看表面措辞。
