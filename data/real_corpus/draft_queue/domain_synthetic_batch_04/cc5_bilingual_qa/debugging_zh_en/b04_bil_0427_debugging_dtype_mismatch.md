...
# Debugging Tensor Shapes and Data Issues 27: Dtype Mismatch

## Concept
Q: dtype 不一致在训练日志里有什么信号？
EN: what dtype mismatches look like in logs?
...
## Explanation
Q: 如果我只从表面文本看，为什么容易误判 dtype mismatch？
EN: Why is dtype mismatch easy to misread if I only inspect the visible text?
...
## Minimal Example
Q: 能给一个围绕 dtype mismatch 的小例子吗？
EN: Can you give a small example around dtype mismatch?
...
## Common Pitfalls
- 把 dtype mismatch 的现象当成模型质量结论，而不是草稿级观察。
- Treating dtype mismatch as a model-quality conclusion instead of a draft-level observation.
