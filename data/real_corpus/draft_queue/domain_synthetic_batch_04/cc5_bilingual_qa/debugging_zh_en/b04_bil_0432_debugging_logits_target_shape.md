...
# Debugging Tensor Shapes and Data Issues 32: Logits Target Shape

## Concept
Q: logits 和 target 维度为何经常错一位？
EN: why logits and targets often differ by one axis?
...
## Explanation
Q: 如果我只从表面文本看，为什么容易误判 logits target shape？
EN: Why is logits target shape easy to misread if I only inspect the visible text?
...
## Minimal Example
Q: 能给一个围绕 logits target shape 的小例子吗？
EN: Can you give a small example around logits target shape?
...
## Common Pitfalls
- 把 logits target shape 的现象当成模型质量结论，而不是草稿级观察。
- Treating logits target shape as a model-quality conclusion instead of a draft-level observation.
