...
"""Tokenizer Boundaries and Mixed Scripts 42: Mixed Punctuation Spacing

ZH: 中英混排里的空格和标点如何影响分词。
...
def build_demo_rows() -> List[Dict[str, object]]:
    ...
    return [
        {
            "topic_id": "B04-BIL-0042",
            "group_theme": "Tokenizer Boundaries and Mixed Scripts",
            "variant": "mixed punctuation spacing",
...
# - 这个 Python 片段围绕 mixed punctuation spacing 构造最小双语检查样例。
# - This Python snippet builds a tiny bilingual inspection example around mixed punctuation spacing.
# - 它不访问网络、不读写真实用户数据，也不运行训练。
# - It does not touch networks, real user data, or training execution.
# - 第 42 个局部样例强调 reviewer 应先看结构，再看表面措辞。
