...
@dataclass
class BatchingHelpersExample046:
    topic_id: str = "B04-COD-0246"
...
def build_batching_helpers_example_046(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "batching_helpers" / "B04-COD-0246.json")


def main() -> None:
    example = BatchingHelpersExample046()
    stats = build_batching_helpers_example_046([46, 47, 48, 49])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0246"
