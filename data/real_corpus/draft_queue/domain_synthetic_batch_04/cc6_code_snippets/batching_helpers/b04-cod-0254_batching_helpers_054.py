...
@dataclass
class BatchingHelpersExample054:
    topic_id: str = "B04-COD-0254"
...
def build_batching_helpers_example_054(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "batching_helpers" / "B04-COD-0254.json")


def main() -> None:
    example = BatchingHelpersExample054()
    stats = build_batching_helpers_example_054([54, 55, 56, 57])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0254"
