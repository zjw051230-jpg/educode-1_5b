...
@dataclass
class BatchingHelpersExample055:
    topic_id: str = "B04-COD-0255"
...
def build_batching_helpers_example_055(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "batching_helpers" / "B04-COD-0255.json")


def main() -> None:
    example = BatchingHelpersExample055()
    stats = build_batching_helpers_example_055([55, 56, 57, 58])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0255"
