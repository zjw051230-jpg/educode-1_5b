...
@dataclass
class BatchingHelpersExample056:
    topic_id: str = "B04-COD-0256"
...
def build_batching_helpers_example_056(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "batching_helpers" / "B04-COD-0256.json")


def main() -> None:
    example = BatchingHelpersExample056()
    stats = build_batching_helpers_example_056([56, 57, 58, 59])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0256"
