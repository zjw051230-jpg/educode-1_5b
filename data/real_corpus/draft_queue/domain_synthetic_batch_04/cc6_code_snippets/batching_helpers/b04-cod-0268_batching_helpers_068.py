...
@dataclass
class BatchingHelpersExample068:
    topic_id: str = "B04-COD-0268"
...
def build_batching_helpers_example_068(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "batching_helpers" / "B04-COD-0268.json")


def main() -> None:
    example = BatchingHelpersExample068()
    stats = build_batching_helpers_example_068([68, 69, 70, 71])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0268"
