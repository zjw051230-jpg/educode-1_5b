...
@dataclass
class BatchingHelpersExample074:
    topic_id: str = "B04-COD-0274"
...
def build_batching_helpers_example_074(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "batching_helpers" / "B04-COD-0274.json")


def main() -> None:
    example = BatchingHelpersExample074()
    stats = build_batching_helpers_example_074([74, 75, 76, 77])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0274"
