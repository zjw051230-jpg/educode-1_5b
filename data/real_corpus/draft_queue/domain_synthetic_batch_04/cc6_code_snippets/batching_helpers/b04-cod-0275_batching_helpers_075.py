...
@dataclass
class BatchingHelpersExample075:
    topic_id: str = "B04-COD-0275"
...
def build_batching_helpers_example_075(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "batching_helpers" / "B04-COD-0275.json")


def main() -> None:
    example = BatchingHelpersExample075()
    stats = build_batching_helpers_example_075([75, 76, 77, 78])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0275"
