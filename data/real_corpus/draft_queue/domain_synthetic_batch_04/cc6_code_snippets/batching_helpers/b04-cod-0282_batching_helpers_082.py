...
@dataclass
class BatchingHelpersExample082:
    topic_id: str = "B04-COD-0282"
...
def build_batching_helpers_example_082(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "batching_helpers" / "B04-COD-0282.json")


def main() -> None:
    example = BatchingHelpersExample082()
    stats = build_batching_helpers_example_082([82, 83, 84, 85])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0282"
