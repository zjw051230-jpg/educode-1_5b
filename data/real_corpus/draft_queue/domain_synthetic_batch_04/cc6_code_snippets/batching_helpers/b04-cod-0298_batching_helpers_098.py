...
"""Synthetic teaching example for batch construction with focus on next-token target shift."""
...
@dataclass
class BatchingHelpersExample098:
    topic_id: str = "B04-COD-0298"
    subtopic: str = "batching_helpers"
    focus: str = "batch construction"
    variant: int = 8
...
def build_batching_helpers_example_098(values: Iterable[int]) -> dict[str, object]:
...
def maybe_make_tensor() -> tuple[int, ...] | None:
    ...
    if "batching_helpers" in ('minimal_pytorch', 'loss_examples', 'validation_loop'):
        sample = sample + 8
    return tuple(sample.shape)


def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "batching_helpers" / "B04-COD-0298.json")


def main() -> None:
    example = BatchingHelpersExample098()
    stats = build_batching_helpers_example_098([98, 99, 100, 101])
    payload = {
        ...
        "extra_focus": "next-token target shift",
    }
    encoded = json.dumps(payload, ensure_ascii=False, sort_keys=True)
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0298"
