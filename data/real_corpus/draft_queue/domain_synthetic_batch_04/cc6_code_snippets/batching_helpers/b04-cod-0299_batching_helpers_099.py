...
"""Synthetic teaching example for batch construction with focus on batch construction."""
...
@dataclass
class BatchingHelpersExample099:
    topic_id: str = "B04-COD-0299"
...
def build_batching_helpers_example_099(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "batching_helpers" / "B04-COD-0299.json")


def main() -> None:
    example = BatchingHelpersExample099()
    stats = build_batching_helpers_example_099([99, 100, 101, 102])
    payload = {
        ...
        "extra_focus": "batch construction",
    }
    encoded = json.dumps(payload, ensure_ascii=False, sort_keys=True)
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0299"
