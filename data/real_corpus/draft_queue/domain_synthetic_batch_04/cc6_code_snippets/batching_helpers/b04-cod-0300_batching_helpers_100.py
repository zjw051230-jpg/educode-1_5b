...
"""Synthetic teaching example for batch construction with focus on sequence chunking."""
...
@dataclass
class BatchingHelpersExample100:
    topic_id: str = "B04-COD-0300"
...
def build_batching_helpers_example_100(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "batching_helpers" / "B04-COD-0300.json")


def main() -> None:
    example = BatchingHelpersExample100()
    stats = build_batching_helpers_example_100([100, 101, 102, 103])
    payload = {
        ...
        "extra_focus": "sequence chunking",
    }
    encoded = json.dumps(payload, ensure_ascii=False, sort_keys=True)
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0300"
