...
@dataclass
class CheckpointMetadataExample036:
    topic_id: str = "B04-COD-0536"
...
def build_checkpoint_metadata_example_036(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "checkpoint_metadata" / "B04-COD-0536.json")


def main() -> None:
    example = CheckpointMetadataExample036()
    stats = build_checkpoint_metadata_example_036([36, 37, 38, 39])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0536"
