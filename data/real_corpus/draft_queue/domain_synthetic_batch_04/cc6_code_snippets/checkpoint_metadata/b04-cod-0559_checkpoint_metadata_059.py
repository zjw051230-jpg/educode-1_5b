...
@dataclass
class CheckpointMetadataExample059:
    topic_id: str = "B04-COD-0559"
...
def build_checkpoint_metadata_example_059(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "checkpoint_metadata" / "B04-COD-0559.json")


def main() -> None:
    example = CheckpointMetadataExample059()
    stats = build_checkpoint_metadata_example_059([59, 60, 61, 62])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0559"
