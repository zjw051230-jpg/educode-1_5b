...
@dataclass
class CheckpointMetadataExample080:
    topic_id: str = "B04-COD-0580"
...
def build_checkpoint_metadata_example_080(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "checkpoint_metadata" / "B04-COD-0580.json")


def main() -> None:
    example = CheckpointMetadataExample080()
    stats = build_checkpoint_metadata_example_080([80, 81, 82, 83])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0580"
