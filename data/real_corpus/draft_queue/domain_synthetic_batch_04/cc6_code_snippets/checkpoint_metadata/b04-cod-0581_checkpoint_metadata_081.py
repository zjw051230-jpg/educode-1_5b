...
@dataclass
class CheckpointMetadataExample081:
    topic_id: str = "B04-COD-0581"
...
def build_checkpoint_metadata_example_081(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "checkpoint_metadata" / "B04-COD-0581.json")


def main() -> None:
    example = CheckpointMetadataExample081()
    stats = build_checkpoint_metadata_example_081([81, 82, 83, 84])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0581"
