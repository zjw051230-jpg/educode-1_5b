...
@dataclass
class CheckpointMetadataExample089:
    topic_id: str = "B04-COD-0589"
...
def build_checkpoint_metadata_example_089(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "checkpoint_metadata" / "B04-COD-0589.json")


def main() -> None:
    example = CheckpointMetadataExample089()
    stats = build_checkpoint_metadata_example_089([89, 90, 91, 92])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0589"
