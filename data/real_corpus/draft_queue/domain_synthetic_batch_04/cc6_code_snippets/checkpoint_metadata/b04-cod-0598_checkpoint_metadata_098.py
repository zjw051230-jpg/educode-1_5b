...
"""Synthetic teaching example for checkpoint metadata with focus on experiment summary writer."""
...
@dataclass
class CheckpointMetadataExample098:
    topic_id: str = "B04-COD-0598"
    subtopic: str = "checkpoint_metadata"
    focus: str = "checkpoint metadata"
    variant: int = 8
...
def build_checkpoint_metadata_example_098(values: Iterable[int]) -> dict[str, object]:
...
def maybe_make_tensor() -> tuple[int, ...] | None:
    ...
    if "checkpoint_metadata" in ('minimal_pytorch', 'loss_examples', 'validation_loop'):
        sample = sample + 8
    return tuple(sample.shape)


def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "checkpoint_metadata" / "B04-COD-0598.json")


def main() -> None:
    example = CheckpointMetadataExample098()
    stats = build_checkpoint_metadata_example_098([98, 99, 100, 101])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0598"
