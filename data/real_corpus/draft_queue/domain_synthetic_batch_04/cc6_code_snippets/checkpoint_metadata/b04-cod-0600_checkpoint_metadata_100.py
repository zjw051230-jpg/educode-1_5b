...
"""Synthetic teaching example for checkpoint metadata with focus on checkpoint reload check."""
...
@dataclass
class CheckpointMetadataExample100:
    topic_id: str = "B04-COD-0600"
...
def build_checkpoint_metadata_example_100(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "checkpoint_metadata" / "B04-COD-0600.json")


def main() -> None:
    example = CheckpointMetadataExample100()
    stats = build_checkpoint_metadata_example_100([100, 101, 102, 103])
    payload = {
        ...
        "extra_focus": "checkpoint reload check",
    }
    encoded = json.dumps(payload, ensure_ascii=False, sort_keys=True)
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0600"
