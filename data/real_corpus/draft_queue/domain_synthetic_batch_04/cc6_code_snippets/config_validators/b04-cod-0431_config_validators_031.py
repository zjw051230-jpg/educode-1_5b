...
@dataclass
class ConfigValidatorsExample031:
    topic_id: str = "B04-COD-0431"
...
def build_config_validators_example_031(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "config_validators" / "B04-COD-0431.json")


def main() -> None:
    example = ConfigValidatorsExample031()
    stats = build_config_validators_example_031([31, 32, 33, 34])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0431"
