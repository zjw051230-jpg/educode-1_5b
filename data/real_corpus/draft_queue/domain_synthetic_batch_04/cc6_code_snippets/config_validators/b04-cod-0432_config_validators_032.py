...
@dataclass
class ConfigValidatorsExample032:
    topic_id: str = "B04-COD-0432"
...
def build_config_validators_example_032(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "config_validators" / "B04-COD-0432.json")


def main() -> None:
    example = ConfigValidatorsExample032()
    stats = build_config_validators_example_032([32, 33, 34, 35])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0432"
