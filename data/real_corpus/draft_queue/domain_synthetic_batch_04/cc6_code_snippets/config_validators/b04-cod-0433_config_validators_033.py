...
@dataclass
class ConfigValidatorsExample033:
    topic_id: str = "B04-COD-0433"
...
def build_config_validators_example_033(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "config_validators" / "B04-COD-0433.json")


def main() -> None:
    example = ConfigValidatorsExample033()
    stats = build_config_validators_example_033([33, 34, 35, 36])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0433"
