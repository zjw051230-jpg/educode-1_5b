...
@dataclass
class ConfigValidatorsExample048:
    topic_id: str = "B04-COD-0448"
...
def build_config_validators_example_048(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "config_validators" / "B04-COD-0448.json")


def main() -> None:
    example = ConfigValidatorsExample048()
    stats = build_config_validators_example_048([48, 49, 50, 51])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0448"
