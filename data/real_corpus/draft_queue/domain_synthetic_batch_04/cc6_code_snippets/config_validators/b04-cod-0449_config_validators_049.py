...
@dataclass
class ConfigValidatorsExample049:
    topic_id: str = "B04-COD-0449"
...
def build_config_validators_example_049(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "config_validators" / "B04-COD-0449.json")


def main() -> None:
    example = ConfigValidatorsExample049()
    stats = build_config_validators_example_049([49, 50, 51, 52])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0449"
