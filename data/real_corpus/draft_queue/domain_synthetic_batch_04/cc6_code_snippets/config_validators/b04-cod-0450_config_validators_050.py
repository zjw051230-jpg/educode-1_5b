...
@dataclass
class ConfigValidatorsExample050:
    topic_id: str = "B04-COD-0450"
...
def build_config_validators_example_050(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "config_validators" / "B04-COD-0450.json")


def main() -> None:
    example = ConfigValidatorsExample050()
    stats = build_config_validators_example_050([50, 51, 52, 53])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0450"
