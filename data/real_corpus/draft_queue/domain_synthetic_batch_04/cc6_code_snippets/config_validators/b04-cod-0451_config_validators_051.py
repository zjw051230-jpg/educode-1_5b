...
@dataclass
class ConfigValidatorsExample051:
    topic_id: str = "B04-COD-0451"
...
def build_config_validators_example_051(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "config_validators" / "B04-COD-0451.json")


def main() -> None:
    example = ConfigValidatorsExample051()
    stats = build_config_validators_example_051([51, 52, 53, 54])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0451"
