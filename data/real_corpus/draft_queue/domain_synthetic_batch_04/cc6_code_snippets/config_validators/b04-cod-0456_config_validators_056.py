...
@dataclass
class ConfigValidatorsExample056:
    topic_id: str = "B04-COD-0456"
...
def build_config_validators_example_056(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "config_validators" / "B04-COD-0456.json")


def main() -> None:
    example = ConfigValidatorsExample056()
    stats = build_config_validators_example_056([56, 57, 58, 59])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0456"
