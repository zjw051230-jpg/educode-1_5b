...
@dataclass
class ConfigValidatorsExample057:
    topic_id: str = "B04-COD-0457"
...
def build_config_validators_example_057(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "config_validators" / "B04-COD-0457.json")


def main() -> None:
    example = ConfigValidatorsExample057()
    stats = build_config_validators_example_057([57, 58, 59, 60])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0457"
