...
@dataclass
class ConfigValidatorsExample059:
    topic_id: str = "B04-COD-0459"
...
def build_config_validators_example_059(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "config_validators" / "B04-COD-0459.json")


def main() -> None:
    example = ConfigValidatorsExample059()
    stats = build_config_validators_example_059([59, 60, 61, 62])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0459"
