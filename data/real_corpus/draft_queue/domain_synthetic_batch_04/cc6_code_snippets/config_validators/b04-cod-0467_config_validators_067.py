...
@dataclass
class ConfigValidatorsExample067:
    topic_id: str = "B04-COD-0467"
...
def build_config_validators_example_067(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "config_validators" / "B04-COD-0467.json")


def main() -> None:
    example = ConfigValidatorsExample067()
    stats = build_config_validators_example_067([67, 68, 69, 70])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0467"
