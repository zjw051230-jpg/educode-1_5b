...
@dataclass
class ConfigValidatorsExample073:
    topic_id: str = "B04-COD-0473"
...
def build_config_validators_example_073(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "config_validators" / "B04-COD-0473.json")


def main() -> None:
    example = ConfigValidatorsExample073()
    stats = build_config_validators_example_073([73, 74, 75, 76])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0473"
