...
@dataclass
class ConfigValidatorsExample074:
    topic_id: str = "B04-COD-0474"
...
def build_config_validators_example_074(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "config_validators" / "B04-COD-0474.json")


def main() -> None:
    example = ConfigValidatorsExample074()
    stats = build_config_validators_example_074([74, 75, 76, 77])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0474"
