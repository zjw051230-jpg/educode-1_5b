...
@dataclass
class ConfigValidatorsExample079:
    topic_id: str = "B04-COD-0479"
...
def build_config_validators_example_079(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "config_validators" / "B04-COD-0479.json")


def main() -> None:
    example = ConfigValidatorsExample079()
    stats = build_config_validators_example_079([79, 80, 81, 82])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0479"
