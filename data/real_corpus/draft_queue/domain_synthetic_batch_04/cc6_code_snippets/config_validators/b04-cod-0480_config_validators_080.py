...
@dataclass
class ConfigValidatorsExample080:
    topic_id: str = "B04-COD-0480"
...
def build_config_validators_example_080(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "config_validators" / "B04-COD-0480.json")


def main() -> None:
    example = ConfigValidatorsExample080()
    stats = build_config_validators_example_080([80, 81, 82, 83])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0480"
