...
@dataclass
class ConfigValidatorsExample097:
    topic_id: str = "B04-COD-0497"
...
def build_config_validators_example_097(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "config_validators" / "B04-COD-0497.json")


def main() -> None:
    example = ConfigValidatorsExample097()
    stats = build_config_validators_example_097([97, 98, 99, 100])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0497"
