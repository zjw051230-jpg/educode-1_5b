...
"""Synthetic teaching example for config validation with focus on safe text cleaning helper."""
...
@dataclass
class ConfigValidatorsExample098:
    topic_id: str = "B04-COD-0498"
...
def build_config_validators_example_098(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "config_validators" / "B04-COD-0498.json")


def main() -> None:
    example = ConfigValidatorsExample098()
    stats = build_config_validators_example_098([98, 99, 100, 101])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0498"
