...
"""Synthetic teaching example for config validation with focus on config validation."""
...
@dataclass
class ConfigValidatorsExample099:
    topic_id: str = "B04-COD-0499"
...
def build_config_validators_example_099(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "config_validators" / "B04-COD-0499.json")


def main() -> None:
    example = ConfigValidatorsExample099()
    stats = build_config_validators_example_099([99, 100, 101, 102])
    payload = {
        ...
        "extra_focus": "config validation",
    }
    encoded = json.dumps(payload, ensure_ascii=False, sort_keys=True)
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0499"
