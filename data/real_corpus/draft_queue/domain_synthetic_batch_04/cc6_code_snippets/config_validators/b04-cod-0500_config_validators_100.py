...
"""Synthetic teaching example for config validation with focus on finite loss check."""
...
@dataclass
class ConfigValidatorsExample100:
    topic_id: str = "B04-COD-0500"
    subtopic: str = "config_validators"
    focus: str = "config validation"
    variant: int = 0
...
def build_config_validators_example_100(values: Iterable[int]) -> dict[str, object]:
...
def maybe_make_tensor() -> tuple[int, ...] | None:
    ...
    if "config_validators" in ('minimal_pytorch', 'loss_examples', 'validation_loop'):
        sample = sample + 0
    return tuple(sample.shape)


def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "config_validators" / "B04-COD-0500.json")


def main() -> None:
    example = ConfigValidatorsExample100()
    stats = build_config_validators_example_100([100, 101, 102, 103])
    payload = {
        ...
        "extra_focus": "finite loss check",
    }
    encoded = json.dumps(payload, ensure_ascii=False, sort_keys=True)
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0500"
