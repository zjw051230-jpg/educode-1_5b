...
@dataclass
class JsonlSanityCheckersExample035:
    topic_id: str = "B04-COD-0935"
...
def build_jsonl_sanity_checkers_example_035(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "jsonl_sanity_checkers" / "B04-COD-0935.json")


def main() -> None:
    example = JsonlSanityCheckersExample035()
    stats = build_jsonl_sanity_checkers_example_035([35, 36, 37, 38])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0935"
