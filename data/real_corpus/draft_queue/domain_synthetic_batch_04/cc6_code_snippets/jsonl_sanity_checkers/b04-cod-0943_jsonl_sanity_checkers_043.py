...
@dataclass
class JsonlSanityCheckersExample043:
    topic_id: str = "B04-COD-0943"
...
def build_jsonl_sanity_checkers_example_043(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "jsonl_sanity_checkers" / "B04-COD-0943.json")


def main() -> None:
    example = JsonlSanityCheckersExample043()
    stats = build_jsonl_sanity_checkers_example_043([43, 44, 45, 46])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0943"
