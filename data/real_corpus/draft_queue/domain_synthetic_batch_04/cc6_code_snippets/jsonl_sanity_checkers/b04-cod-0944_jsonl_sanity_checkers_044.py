...
@dataclass
class JsonlSanityCheckersExample044:
    topic_id: str = "B04-COD-0944"
...
def build_jsonl_sanity_checkers_example_044(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "jsonl_sanity_checkers" / "B04-COD-0944.json")


def main() -> None:
    example = JsonlSanityCheckersExample044()
    stats = build_jsonl_sanity_checkers_example_044([44, 45, 46, 47])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0944"
