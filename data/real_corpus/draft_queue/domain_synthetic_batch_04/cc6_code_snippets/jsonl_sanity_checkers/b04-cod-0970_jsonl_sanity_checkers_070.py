...
@dataclass
class JsonlSanityCheckersExample070:
    topic_id: str = "B04-COD-0970"
...
def build_jsonl_sanity_checkers_example_070(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "jsonl_sanity_checkers" / "B04-COD-0970.json")


def main() -> None:
    example = JsonlSanityCheckersExample070()
    stats = build_jsonl_sanity_checkers_example_070([70, 71, 72, 73])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0970"
