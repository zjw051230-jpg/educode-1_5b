...
@dataclass
class JsonlSanityCheckersExample080:
    topic_id: str = "B04-COD-0980"
...
def build_jsonl_sanity_checkers_example_080(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "jsonl_sanity_checkers" / "B04-COD-0980.json")


def main() -> None:
    example = JsonlSanityCheckersExample080()
    stats = build_jsonl_sanity_checkers_example_080([80, 81, 82, 83])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0980"
