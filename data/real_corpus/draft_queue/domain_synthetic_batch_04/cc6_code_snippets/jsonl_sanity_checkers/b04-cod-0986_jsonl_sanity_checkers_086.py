...
@dataclass
class JsonlSanityCheckersExample086:
    topic_id: str = "B04-COD-0986"
...
def build_jsonl_sanity_checkers_example_086(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "jsonl_sanity_checkers" / "B04-COD-0986.json")


def main() -> None:
    example = JsonlSanityCheckersExample086()
    stats = build_jsonl_sanity_checkers_example_086([86, 87, 88, 89])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0986"
