...
@dataclass
class JsonlSanityCheckersExample090:
    topic_id: str = "B04-COD-0990"
...
def build_jsonl_sanity_checkers_example_090(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "jsonl_sanity_checkers" / "B04-COD-0990.json")


def main() -> None:
    example = JsonlSanityCheckersExample090()
    stats = build_jsonl_sanity_checkers_example_090([90, 91, 92, 93])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0990"
