...
"""Synthetic teaching example for JSONL sanity checker with focus on metrics reader."""
...
@dataclass
class JsonlSanityCheckersExample098:
    topic_id: str = "B04-COD-0998"
...
def build_jsonl_sanity_checkers_example_098(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "jsonl_sanity_checkers" / "B04-COD-0998.json")


def main() -> None:
    example = JsonlSanityCheckersExample098()
    stats = build_jsonl_sanity_checkers_example_098([98, 99, 100, 101])
    payload = {
        ...
        "extra_focus": "metrics reader",
    }
    encoded = json.dumps(payload, ensure_ascii=False, sort_keys=True)
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0998"
