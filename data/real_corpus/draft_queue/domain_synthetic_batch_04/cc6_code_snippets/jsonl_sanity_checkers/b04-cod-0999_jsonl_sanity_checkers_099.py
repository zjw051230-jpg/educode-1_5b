...
"""Synthetic teaching example for JSONL sanity checker with focus on JSONL sanity checker."""
...
@dataclass
class JsonlSanityCheckersExample099:
    topic_id: str = "B04-COD-0999"
    subtopic: str = "jsonl_sanity_checkers"
    focus: str = "JSONL sanity checker"
    variant: int = 9
...
def build_jsonl_sanity_checkers_example_099(values: Iterable[int]) -> dict[str, object]:
...
def maybe_make_tensor() -> tuple[int, ...] | None:
    ...
    if "jsonl_sanity_checkers" in ('minimal_pytorch', 'loss_examples', 'validation_loop'):
        sample = sample + 9
    return tuple(sample.shape)


def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "jsonl_sanity_checkers" / "B04-COD-0999.json")


def main() -> None:
    example = JsonlSanityCheckersExample099()
    stats = build_jsonl_sanity_checkers_example_099([99, 100, 101, 102])
    payload = {
        ...
        "extra_focus": "JSONL sanity checker",
    }
    encoded = json.dumps(payload, ensure_ascii=False, sort_keys=True)
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0999"
