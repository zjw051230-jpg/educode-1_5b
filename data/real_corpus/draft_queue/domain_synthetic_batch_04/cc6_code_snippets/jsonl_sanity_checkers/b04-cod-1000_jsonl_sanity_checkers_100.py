...
"""Synthetic teaching example for JSONL sanity checker with focus on safe text cleaning helper."""
...
@dataclass
class JsonlSanityCheckersExample100:
    topic_id: str = "B04-COD-1000"
...
def build_jsonl_sanity_checkers_example_100(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "jsonl_sanity_checkers" / "B04-COD-1000.json")


def main() -> None:
    example = JsonlSanityCheckersExample100()
    stats = build_jsonl_sanity_checkers_example_100([100, 101, 102, 103])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-1000"
