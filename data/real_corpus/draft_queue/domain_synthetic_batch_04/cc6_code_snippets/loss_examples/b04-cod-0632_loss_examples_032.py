...
@dataclass
class LossExamplesExample032:
    topic_id: str = "B04-COD-0632"
...
def build_loss_examples_example_032(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "loss_examples" / "B04-COD-0632.json")


def main() -> None:
    example = LossExamplesExample032()
    stats = build_loss_examples_example_032([32, 33, 34, 35])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0632"
