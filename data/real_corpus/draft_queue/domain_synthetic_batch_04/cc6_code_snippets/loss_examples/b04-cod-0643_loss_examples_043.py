...
@dataclass
class LossExamplesExample043:
    topic_id: str = "B04-COD-0643"
...
def build_loss_examples_example_043(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "loss_examples" / "B04-COD-0643.json")


def main() -> None:
    example = LossExamplesExample043()
    stats = build_loss_examples_example_043([43, 44, 45, 46])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0643"
