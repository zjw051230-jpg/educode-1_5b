...
@dataclass
class LossExamplesExample045:
    topic_id: str = "B04-COD-0645"
...
def build_loss_examples_example_045(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "loss_examples" / "B04-COD-0645.json")


def main() -> None:
    example = LossExamplesExample045()
    stats = build_loss_examples_example_045([45, 46, 47, 48])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0645"
