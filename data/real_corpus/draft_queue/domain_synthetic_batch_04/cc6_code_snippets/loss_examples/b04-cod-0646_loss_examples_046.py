...
@dataclass
class LossExamplesExample046:
    topic_id: str = "B04-COD-0646"
...
def build_loss_examples_example_046(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "loss_examples" / "B04-COD-0646.json")


def main() -> None:
    example = LossExamplesExample046()
    stats = build_loss_examples_example_046([46, 47, 48, 49])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0646"
