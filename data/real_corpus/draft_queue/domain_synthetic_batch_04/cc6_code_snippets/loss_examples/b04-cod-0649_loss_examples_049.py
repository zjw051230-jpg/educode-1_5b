...
@dataclass
class LossExamplesExample049:
    topic_id: str = "B04-COD-0649"
...
def build_loss_examples_example_049(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "loss_examples" / "B04-COD-0649.json")


def main() -> None:
    example = LossExamplesExample049()
    stats = build_loss_examples_example_049([49, 50, 51, 52])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0649"
