...
@dataclass
class LossExamplesExample055:
    topic_id: str = "B04-COD-0655"
...
def build_loss_examples_example_055(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "loss_examples" / "B04-COD-0655.json")


def main() -> None:
    example = LossExamplesExample055()
    stats = build_loss_examples_example_055([55, 56, 57, 58])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0655"
