...
@dataclass
class LossExamplesExample061:
    topic_id: str = "B04-COD-0661"
...
def build_loss_examples_example_061(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "loss_examples" / "B04-COD-0661.json")


def main() -> None:
    example = LossExamplesExample061()
    stats = build_loss_examples_example_061([61, 62, 63, 64])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0661"
