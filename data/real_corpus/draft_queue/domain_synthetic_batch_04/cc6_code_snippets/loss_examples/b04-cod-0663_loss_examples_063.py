...
@dataclass
class LossExamplesExample063:
    topic_id: str = "B04-COD-0663"
...
def build_loss_examples_example_063(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "loss_examples" / "B04-COD-0663.json")


def main() -> None:
    example = LossExamplesExample063()
    stats = build_loss_examples_example_063([63, 64, 65, 66])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0663"
