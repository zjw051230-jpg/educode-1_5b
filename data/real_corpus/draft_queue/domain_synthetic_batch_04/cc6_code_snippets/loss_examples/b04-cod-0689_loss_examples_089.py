...
@dataclass
class LossExamplesExample089:
    topic_id: str = "B04-COD-0689"
...
def build_loss_examples_example_089(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "loss_examples" / "B04-COD-0689.json")


def main() -> None:
    example = LossExamplesExample089()
    stats = build_loss_examples_example_089([89, 90, 91, 92])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0689"
