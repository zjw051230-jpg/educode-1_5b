...
@dataclass
class LossExamplesExample091:
    topic_id: str = "B04-COD-0691"
...
def build_loss_examples_example_091(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "loss_examples" / "B04-COD-0691.json")


def main() -> None:
    example = LossExamplesExample091()
    stats = build_loss_examples_example_091([91, 92, 93, 94])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0691"
