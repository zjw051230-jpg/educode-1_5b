...
"""Synthetic teaching example for cross entropy example with focus on logits shape check."""
...
@dataclass
class LossExamplesExample098:
    topic_id: str = "B04-COD-0698"
...
def build_loss_examples_example_098(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "loss_examples" / "B04-COD-0698.json")


def main() -> None:
    example = LossExamplesExample098()
    stats = build_loss_examples_example_098([98, 99, 100, 101])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0698"
