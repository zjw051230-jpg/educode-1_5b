...
"""Synthetic teaching example for cross entropy example with focus on cross entropy example."""
...
@dataclass
class LossExamplesExample099:
    topic_id: str = "B04-COD-0699"
...
def build_loss_examples_example_099(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "loss_examples" / "B04-COD-0699.json")


def main() -> None:
    example = LossExamplesExample099()
    stats = build_loss_examples_example_099([99, 100, 101, 102])
    payload = {
        ...
        "extra_focus": "cross entropy example",
    }
    encoded = json.dumps(payload, ensure_ascii=False, sort_keys=True)
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0699"
