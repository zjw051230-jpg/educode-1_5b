...
"""Synthetic teaching example for cross entropy example with focus on next-token target shift."""
...
@dataclass
class LossExamplesExample100:
    topic_id: str = "B04-COD-0700"
    subtopic: str = "loss_examples"
    focus: str = "cross entropy example"
    variant: int = 0
...
def build_loss_examples_example_100(values: Iterable[int]) -> dict[str, object]:
...
def maybe_make_tensor() -> tuple[int, ...] | None:
    ...
    if "loss_examples" in ('minimal_pytorch', 'loss_examples', 'validation_loop'):
        sample = sample + 0
    return tuple(sample.shape)


def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "loss_examples" / "B04-COD-0700.json")


def main() -> None:
    example = LossExamplesExample100()
    stats = build_loss_examples_example_100([100, 101, 102, 103])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0700"
