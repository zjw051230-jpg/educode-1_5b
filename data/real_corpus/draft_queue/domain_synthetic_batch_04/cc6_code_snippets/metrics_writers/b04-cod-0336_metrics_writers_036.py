...
@dataclass
class MetricsWritersExample036:
    topic_id: str = "B04-COD-0336"
...
def build_metrics_writers_example_036(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "metrics_writers" / "B04-COD-0336.json")


def main() -> None:
    example = MetricsWritersExample036()
    stats = build_metrics_writers_example_036([36, 37, 38, 39])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0336"
