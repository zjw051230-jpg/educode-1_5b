...
@dataclass
class MetricsWritersExample050:
    topic_id: str = "B04-COD-0350"
...
def build_metrics_writers_example_050(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "metrics_writers" / "B04-COD-0350.json")


def main() -> None:
    example = MetricsWritersExample050()
    stats = build_metrics_writers_example_050([50, 51, 52, 53])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0350"
