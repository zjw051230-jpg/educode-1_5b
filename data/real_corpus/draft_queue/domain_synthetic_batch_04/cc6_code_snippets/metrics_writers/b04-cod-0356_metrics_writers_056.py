...
@dataclass
class MetricsWritersExample056:
    topic_id: str = "B04-COD-0356"
...
def build_metrics_writers_example_056(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "metrics_writers" / "B04-COD-0356.json")


def main() -> None:
    example = MetricsWritersExample056()
    stats = build_metrics_writers_example_056([56, 57, 58, 59])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0356"
