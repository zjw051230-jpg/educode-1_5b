...
@dataclass
class MetricsWritersExample057:
    topic_id: str = "B04-COD-0357"
...
def build_metrics_writers_example_057(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "metrics_writers" / "B04-COD-0357.json")


def main() -> None:
    example = MetricsWritersExample057()
    stats = build_metrics_writers_example_057([57, 58, 59, 60])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0357"
