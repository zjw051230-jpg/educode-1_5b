...
@dataclass
class MetricsWritersExample062:
    topic_id: str = "B04-COD-0362"
...
def build_metrics_writers_example_062(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "metrics_writers" / "B04-COD-0362.json")


def main() -> None:
    example = MetricsWritersExample062()
    stats = build_metrics_writers_example_062([62, 63, 64, 65])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0362"
