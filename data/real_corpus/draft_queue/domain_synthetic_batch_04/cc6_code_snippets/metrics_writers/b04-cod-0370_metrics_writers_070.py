...
@dataclass
class MetricsWritersExample070:
    topic_id: str = "B04-COD-0370"
...
def build_metrics_writers_example_070(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "metrics_writers" / "B04-COD-0370.json")


def main() -> None:
    example = MetricsWritersExample070()
    stats = build_metrics_writers_example_070([70, 71, 72, 73])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0370"
