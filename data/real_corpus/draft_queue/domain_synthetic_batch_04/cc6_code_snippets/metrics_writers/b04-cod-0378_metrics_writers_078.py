...
@dataclass
class MetricsWritersExample078:
    topic_id: str = "B04-COD-0378"
...
def build_metrics_writers_example_078(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "metrics_writers" / "B04-COD-0378.json")


def main() -> None:
    example = MetricsWritersExample078()
    stats = build_metrics_writers_example_078([78, 79, 80, 81])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0378"
