...
@dataclass
class MetricsWritersExample081:
    topic_id: str = "B04-COD-0381"
...
def build_metrics_writers_example_081(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "metrics_writers" / "B04-COD-0381.json")


def main() -> None:
    example = MetricsWritersExample081()
    stats = build_metrics_writers_example_081([81, 82, 83, 84])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0381"
