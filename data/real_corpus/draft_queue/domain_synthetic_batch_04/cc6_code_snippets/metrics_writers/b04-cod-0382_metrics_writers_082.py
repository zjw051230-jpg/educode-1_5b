...
@dataclass
class MetricsWritersExample082:
    topic_id: str = "B04-COD-0382"
...
def build_metrics_writers_example_082(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "metrics_writers" / "B04-COD-0382.json")


def main() -> None:
    example = MetricsWritersExample082()
    stats = build_metrics_writers_example_082([82, 83, 84, 85])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0382"
