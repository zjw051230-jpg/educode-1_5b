...
"""Synthetic teaching example for metrics writer with focus on experiment summary writer."""
...
@dataclass
class MetricsWritersExample098:
    topic_id: str = "B04-COD-0398"
    subtopic: str = "metrics_writers"
    focus: str = "metrics writer"
    variant: int = 8
...
def build_metrics_writers_example_098(values: Iterable[int]) -> dict[str, object]:
...
def maybe_make_tensor() -> tuple[int, ...] | None:
    ...
    if "metrics_writers" in ('minimal_pytorch', 'loss_examples', 'validation_loop'):
        sample = sample + 8
    return tuple(sample.shape)


def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "metrics_writers" / "B04-COD-0398.json")


def main() -> None:
    example = MetricsWritersExample098()
    stats = build_metrics_writers_example_098([98, 99, 100, 101])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0398"
