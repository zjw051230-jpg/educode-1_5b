...
"""Synthetic teaching example for metrics writer with focus on metrics writer."""
...
@dataclass
class MetricsWritersExample099:
    topic_id: str = "B04-COD-0399"
...
def build_metrics_writers_example_099(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "metrics_writers" / "B04-COD-0399.json")


def main() -> None:
    example = MetricsWritersExample099()
    stats = build_metrics_writers_example_099([99, 100, 101, 102])
    payload = {
        ...
        "extra_focus": "metrics writer",
    }
    encoded = json.dumps(payload, ensure_ascii=False, sort_keys=True)
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0399"
