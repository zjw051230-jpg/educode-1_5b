...
"""Synthetic teaching example for metrics writer with focus on metrics reader."""
...
@dataclass
class MetricsWritersExample100:
    topic_id: str = "B04-COD-0400"
...
def build_metrics_writers_example_100(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "metrics_writers" / "B04-COD-0400.json")


def main() -> None:
    example = MetricsWritersExample100()
    stats = build_metrics_writers_example_100([100, 101, 102, 103])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0400"
