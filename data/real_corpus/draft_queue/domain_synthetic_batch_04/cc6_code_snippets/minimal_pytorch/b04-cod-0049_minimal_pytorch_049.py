...
@dataclass
class MinimalPytorchExample049:
    topic_id: str = "B04-COD-0049"
...
def build_minimal_pytorch_example_049(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "minimal_pytorch" / "B04-COD-0049.json")


def main() -> None:
    example = MinimalPytorchExample049()
    stats = build_minimal_pytorch_example_049([49, 50, 51, 52])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0049"
