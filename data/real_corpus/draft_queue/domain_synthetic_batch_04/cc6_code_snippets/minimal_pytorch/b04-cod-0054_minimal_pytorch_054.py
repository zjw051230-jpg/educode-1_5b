...
@dataclass
class MinimalPytorchExample054:
    topic_id: str = "B04-COD-0054"
...
def build_minimal_pytorch_example_054(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "minimal_pytorch" / "B04-COD-0054.json")


def main() -> None:
    example = MinimalPytorchExample054()
    stats = build_minimal_pytorch_example_054([54, 55, 56, 57])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0054"
