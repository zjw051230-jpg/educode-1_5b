...
@dataclass
class MinimalPytorchExample062:
    topic_id: str = "B04-COD-0062"
...
def build_minimal_pytorch_example_062(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "minimal_pytorch" / "B04-COD-0062.json")


def main() -> None:
    example = MinimalPytorchExample062()
    stats = build_minimal_pytorch_example_062([62, 63, 64, 65])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0062"
