...
@dataclass
class MinimalPytorchExample072:
    topic_id: str = "B04-COD-0072"
...
def build_minimal_pytorch_example_072(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "minimal_pytorch" / "B04-COD-0072.json")


def main() -> None:
    example = MinimalPytorchExample072()
    stats = build_minimal_pytorch_example_072([72, 73, 74, 75])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0072"
