...
@dataclass
class MinimalPytorchExample082:
    topic_id: str = "B04-COD-0082"
...
def build_minimal_pytorch_example_082(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "minimal_pytorch" / "B04-COD-0082.json")


def main() -> None:
    example = MinimalPytorchExample082()
    stats = build_minimal_pytorch_example_082([82, 83, 84, 85])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0082"
