...
@dataclass
class MinimalPytorchExample085:
    topic_id: str = "B04-COD-0085"
...
def build_minimal_pytorch_example_085(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "minimal_pytorch" / "B04-COD-0085.json")


def main() -> None:
    example = MinimalPytorchExample085()
    stats = build_minimal_pytorch_example_085([85, 86, 87, 88])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0085"
