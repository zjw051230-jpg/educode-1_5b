...
@dataclass
class MinimalPytorchExample087:
    topic_id: str = "B04-COD-0087"
...
def build_minimal_pytorch_example_087(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "minimal_pytorch" / "B04-COD-0087.json")


def main() -> None:
    example = MinimalPytorchExample087()
    stats = build_minimal_pytorch_example_087([87, 88, 89, 90])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0087"
