...
@dataclass
class MinimalPytorchExample088:
    topic_id: str = "B04-COD-0088"
...
def build_minimal_pytorch_example_088(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "minimal_pytorch" / "B04-COD-0088.json")


def main() -> None:
    example = MinimalPytorchExample088()
    stats = build_minimal_pytorch_example_088([88, 89, 90, 91])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0088"
