...
@dataclass
class MinimalPytorchExample090:
    topic_id: str = "B04-COD-0090"
...
def build_minimal_pytorch_example_090(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "minimal_pytorch" / "B04-COD-0090.json")


def main() -> None:
    example = MinimalPytorchExample090()
    stats = build_minimal_pytorch_example_090([90, 91, 92, 93])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0090"
