...
"""Synthetic teaching example for minimal PyTorch module with focus on finite loss check."""
...
@dataclass
class MinimalPytorchExample098:
    topic_id: str = "B04-COD-0098"
...
def build_minimal_pytorch_example_098(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "minimal_pytorch" / "B04-COD-0098.json")


def main() -> None:
    example = MinimalPytorchExample098()
    stats = build_minimal_pytorch_example_098([98, 99, 100, 101])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0098"
