...
"""Synthetic teaching example for minimal PyTorch module with focus on tiny embedding demo."""
...
@dataclass
class MinimalPytorchExample099:
    topic_id: str = "B04-COD-0099"
    subtopic: str = "minimal_pytorch"
    focus: str = "minimal PyTorch module"
    variant: int = 9
...
def build_minimal_pytorch_example_099(values: Iterable[int]) -> dict[str, object]:
...
def maybe_make_tensor() -> tuple[int, ...] | None:
    ...
    if "minimal_pytorch" in ('minimal_pytorch', 'loss_examples', 'validation_loop'):
        sample = sample + 9
    return tuple(sample.shape)


def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "minimal_pytorch" / "B04-COD-0099.json")


def main() -> None:
    example = MinimalPytorchExample099()
    stats = build_minimal_pytorch_example_099([99, 100, 101, 102])
    payload = {
        ...
        "extra_focus": "tiny embedding demo",
    }
    encoded = json.dumps(payload, ensure_ascii=False, sort_keys=True)
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0099"
