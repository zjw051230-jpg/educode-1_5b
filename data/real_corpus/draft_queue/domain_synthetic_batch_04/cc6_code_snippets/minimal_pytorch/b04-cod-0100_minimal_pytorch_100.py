...
"""Synthetic teaching example for minimal PyTorch module with focus on logits shape check."""
...
@dataclass
class MinimalPytorchExample100:
    topic_id: str = "B04-COD-0100"
...
def build_minimal_pytorch_example_100(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "minimal_pytorch" / "B04-COD-0100.json")


def main() -> None:
    example = MinimalPytorchExample100()
    stats = build_minimal_pytorch_example_100([100, 101, 102, 103])
    payload = {
        ...
        "extra_focus": "logits shape check",
    }
    encoded = json.dumps(payload, ensure_ascii=False, sort_keys=True)
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0100"
