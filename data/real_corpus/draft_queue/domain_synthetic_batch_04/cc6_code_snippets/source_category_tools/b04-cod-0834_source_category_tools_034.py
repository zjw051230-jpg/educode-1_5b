...
@dataclass
class SourceCategoryToolsExample034:
    topic_id: str = "B04-COD-0834"
...
def build_source_category_tools_example_034(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "source_category_tools" / "B04-COD-0834.json")


def main() -> None:
    example = SourceCategoryToolsExample034()
    stats = build_source_category_tools_example_034([34, 35, 36, 37])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0834"
