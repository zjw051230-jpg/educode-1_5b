...
@dataclass
class SourceCategoryToolsExample039:
    topic_id: str = "B04-COD-0839"
...
def build_source_category_tools_example_039(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "source_category_tools" / "B04-COD-0839.json")


def main() -> None:
    example = SourceCategoryToolsExample039()
    stats = build_source_category_tools_example_039([39, 40, 41, 42])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0839"
