...
@dataclass
class SourceCategoryToolsExample043:
    topic_id: str = "B04-COD-0843"
...
def build_source_category_tools_example_043(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "source_category_tools" / "B04-COD-0843.json")


def main() -> None:
    example = SourceCategoryToolsExample043()
    stats = build_source_category_tools_example_043([43, 44, 45, 46])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0843"
