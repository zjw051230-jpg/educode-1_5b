...
@dataclass
class SourceCategoryToolsExample045:
    topic_id: str = "B04-COD-0845"
...
def build_source_category_tools_example_045(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "source_category_tools" / "B04-COD-0845.json")


def main() -> None:
    example = SourceCategoryToolsExample045()
    stats = build_source_category_tools_example_045([45, 46, 47, 48])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0845"
