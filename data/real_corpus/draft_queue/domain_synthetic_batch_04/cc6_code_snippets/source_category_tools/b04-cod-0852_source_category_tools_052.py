...
@dataclass
class SourceCategoryToolsExample052:
    topic_id: str = "B04-COD-0852"
...
def build_source_category_tools_example_052(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "source_category_tools" / "B04-COD-0852.json")


def main() -> None:
    example = SourceCategoryToolsExample052()
    stats = build_source_category_tools_example_052([52, 53, 54, 55])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0852"
