...
@dataclass
class SourceCategoryToolsExample055:
    topic_id: str = "B04-COD-0855"
...
def build_source_category_tools_example_055(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "source_category_tools" / "B04-COD-0855.json")


def main() -> None:
    example = SourceCategoryToolsExample055()
    stats = build_source_category_tools_example_055([55, 56, 57, 58])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0855"
