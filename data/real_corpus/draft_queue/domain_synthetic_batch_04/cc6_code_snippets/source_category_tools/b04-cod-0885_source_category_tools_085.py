...
@dataclass
class SourceCategoryToolsExample085:
    topic_id: str = "B04-COD-0885"
...
def build_source_category_tools_example_085(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "source_category_tools" / "B04-COD-0885.json")


def main() -> None:
    example = SourceCategoryToolsExample085()
    stats = build_source_category_tools_example_085([85, 86, 87, 88])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0885"
