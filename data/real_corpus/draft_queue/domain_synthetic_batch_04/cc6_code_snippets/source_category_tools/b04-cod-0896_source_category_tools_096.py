...
@dataclass
class SourceCategoryToolsExample096:
    topic_id: str = "B04-COD-0896"
...
def build_source_category_tools_example_096(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "source_category_tools" / "B04-COD-0896.json")


def main() -> None:
    example = SourceCategoryToolsExample096()
    stats = build_source_category_tools_example_096([96, 97, 98, 99])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0896"
