...
@dataclass
class SourceCategoryToolsExample097:
    topic_id: str = "B04-COD-0897"
...
def build_source_category_tools_example_097(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "source_category_tools" / "B04-COD-0897.json")


def main() -> None:
    example = SourceCategoryToolsExample097()
    stats = build_source_category_tools_example_097([97, 98, 99, 100])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0897"
