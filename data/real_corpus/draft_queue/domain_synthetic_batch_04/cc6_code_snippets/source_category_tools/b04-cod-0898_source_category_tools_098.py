...
"""Synthetic teaching example for source_category counter with focus on deterministic split helper."""
...
@dataclass
class SourceCategoryToolsExample098:
    topic_id: str = "B04-COD-0898"
...
def build_source_category_tools_example_098(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "source_category_tools" / "B04-COD-0898.json")


def main() -> None:
    example = SourceCategoryToolsExample098()
    stats = build_source_category_tools_example_098([98, 99, 100, 101])
    payload = {
        ...
        "extra_focus": "deterministic split helper",
    }
    encoded = json.dumps(payload, ensure_ascii=False, sort_keys=True)
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0898"
