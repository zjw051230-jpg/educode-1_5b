...
"""Synthetic teaching example for source_category counter with focus on source_category counter."""
...
@dataclass
class SourceCategoryToolsExample099:
    topic_id: str = "B04-COD-0899"
...
def build_source_category_tools_example_099(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "source_category_tools" / "B04-COD-0899.json")


def main() -> None:
    example = SourceCategoryToolsExample099()
    stats = build_source_category_tools_example_099([99, 100, 101, 102])
    payload = {
        ...
        "extra_focus": "source_category counter",
    }
    encoded = json.dumps(payload, ensure_ascii=False, sort_keys=True)
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0899"
