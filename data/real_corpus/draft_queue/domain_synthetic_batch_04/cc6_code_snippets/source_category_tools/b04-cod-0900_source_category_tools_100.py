...
"""Synthetic teaching example for source_category counter with focus on experiment summary writer."""
...
@dataclass
class SourceCategoryToolsExample100:
    topic_id: str = "B04-COD-0900"
    subtopic: str = "source_category_tools"
    focus: str = "source_category counter"
    variant: int = 0
...
def build_source_category_tools_example_100(values: Iterable[int]) -> dict[str, object]:
...
def maybe_make_tensor() -> tuple[int, ...] | None:
    ...
    if "source_category_tools" in ('minimal_pytorch', 'loss_examples', 'validation_loop'):
        sample = sample + 0
    return tuple(sample.shape)


def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "source_category_tools" / "B04-COD-0900.json")


def main() -> None:
    example = SourceCategoryToolsExample100()
    stats = build_source_category_tools_example_100([100, 101, 102, 103])
    payload = {
        ...
        "extra_focus": "experiment summary writer",
    }
    encoded = json.dumps(payload, ensure_ascii=False, sort_keys=True)
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0900"
