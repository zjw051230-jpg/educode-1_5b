...
@dataclass
class TokenizerUtilsExample033:
    topic_id: str = "B04-COD-0133"
...
def build_tokenizer_utils_example_033(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "tokenizer_utils" / "B04-COD-0133.json")


def main() -> None:
    example = TokenizerUtilsExample033()
    stats = build_tokenizer_utils_example_033([33, 34, 35, 36])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0133"
