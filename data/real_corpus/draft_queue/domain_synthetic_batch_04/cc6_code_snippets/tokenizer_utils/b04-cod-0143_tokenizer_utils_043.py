...
@dataclass
class TokenizerUtilsExample043:
    topic_id: str = "B04-COD-0143"
...
def build_tokenizer_utils_example_043(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "tokenizer_utils" / "B04-COD-0143.json")


def main() -> None:
    example = TokenizerUtilsExample043()
    stats = build_tokenizer_utils_example_043([43, 44, 45, 46])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0143"
