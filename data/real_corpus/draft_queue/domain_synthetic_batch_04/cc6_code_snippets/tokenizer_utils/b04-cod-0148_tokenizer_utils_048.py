...
@dataclass
class TokenizerUtilsExample048:
    topic_id: str = "B04-COD-0148"
...
def build_tokenizer_utils_example_048(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "tokenizer_utils" / "B04-COD-0148.json")


def main() -> None:
    example = TokenizerUtilsExample048()
    stats = build_tokenizer_utils_example_048([48, 49, 50, 51])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0148"
