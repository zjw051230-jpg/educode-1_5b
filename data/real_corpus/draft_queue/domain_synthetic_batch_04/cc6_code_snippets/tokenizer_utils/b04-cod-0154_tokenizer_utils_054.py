...
@dataclass
class TokenizerUtilsExample054:
    topic_id: str = "B04-COD-0154"
...
def build_tokenizer_utils_example_054(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "tokenizer_utils" / "B04-COD-0154.json")


def main() -> None:
    example = TokenizerUtilsExample054()
    stats = build_tokenizer_utils_example_054([54, 55, 56, 57])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0154"
