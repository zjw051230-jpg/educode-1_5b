...
@dataclass
class TokenizerUtilsExample061:
    topic_id: str = "B04-COD-0161"
...
def build_tokenizer_utils_example_061(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "tokenizer_utils" / "B04-COD-0161.json")


def main() -> None:
    example = TokenizerUtilsExample061()
    stats = build_tokenizer_utils_example_061([61, 62, 63, 64])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0161"
