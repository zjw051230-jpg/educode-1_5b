...
@dataclass
class TokenizerUtilsExample065:
    topic_id: str = "B04-COD-0165"
...
def build_tokenizer_utils_example_065(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "tokenizer_utils" / "B04-COD-0165.json")


def main() -> None:
    example = TokenizerUtilsExample065()
    stats = build_tokenizer_utils_example_065([65, 66, 67, 68])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0165"
