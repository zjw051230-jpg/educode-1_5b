...
@dataclass
class TokenizerUtilsExample074:
    topic_id: str = "B04-COD-0174"
...
def build_tokenizer_utils_example_074(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "tokenizer_utils" / "B04-COD-0174.json")


def main() -> None:
    example = TokenizerUtilsExample074()
    stats = build_tokenizer_utils_example_074([74, 75, 76, 77])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0174"
