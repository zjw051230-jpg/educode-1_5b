...
@dataclass
class TokenizerUtilsExample075:
    topic_id: str = "B04-COD-0175"
...
def build_tokenizer_utils_example_075(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "tokenizer_utils" / "B04-COD-0175.json")


def main() -> None:
    example = TokenizerUtilsExample075()
    stats = build_tokenizer_utils_example_075([75, 76, 77, 78])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0175"
