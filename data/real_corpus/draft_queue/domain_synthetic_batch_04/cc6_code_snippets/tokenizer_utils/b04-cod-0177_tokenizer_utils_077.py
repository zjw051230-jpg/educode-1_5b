...
@dataclass
class TokenizerUtilsExample077:
    topic_id: str = "B04-COD-0177"
...
def build_tokenizer_utils_example_077(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "tokenizer_utils" / "B04-COD-0177.json")


def main() -> None:
    example = TokenizerUtilsExample077()
    stats = build_tokenizer_utils_example_077([77, 78, 79, 80])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0177"
