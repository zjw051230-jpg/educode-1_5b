...
@dataclass
class TokenizerUtilsExample086:
    topic_id: str = "B04-COD-0186"
...
def build_tokenizer_utils_example_086(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "tokenizer_utils" / "B04-COD-0186.json")


def main() -> None:
    example = TokenizerUtilsExample086()
    stats = build_tokenizer_utils_example_086([86, 87, 88, 89])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0186"
