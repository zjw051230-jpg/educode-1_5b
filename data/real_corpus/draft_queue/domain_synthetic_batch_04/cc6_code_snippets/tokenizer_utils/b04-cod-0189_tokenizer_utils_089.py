...
@dataclass
class TokenizerUtilsExample089:
    topic_id: str = "B04-COD-0189"
...
def build_tokenizer_utils_example_089(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "tokenizer_utils" / "B04-COD-0189.json")


def main() -> None:
    example = TokenizerUtilsExample089()
    stats = build_tokenizer_utils_example_089([89, 90, 91, 92])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0189"
