...
@dataclass
class TokenizerUtilsExample096:
    topic_id: str = "B04-COD-0196"
...
def build_tokenizer_utils_example_096(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "tokenizer_utils" / "B04-COD-0196.json")


def main() -> None:
    example = TokenizerUtilsExample096()
    stats = build_tokenizer_utils_example_096([96, 97, 98, 99])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0196"
