...
"""Synthetic teaching example for tokenizer encode/decode helper with focus on tokenizer encode/decode helper."""
...
@dataclass
class TokenizerUtilsExample098:
    topic_id: str = "B04-COD-0198"
...
def build_tokenizer_utils_example_098(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "tokenizer_utils" / "B04-COD-0198.json")


def main() -> None:
    example = TokenizerUtilsExample098()
    stats = build_tokenizer_utils_example_098([98, 99, 100, 101])
    payload = {
        ...
        "extra_focus": "tokenizer encode/decode helper",
    }
    encoded = json.dumps(payload, ensure_ascii=False, sort_keys=True)
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0198"
