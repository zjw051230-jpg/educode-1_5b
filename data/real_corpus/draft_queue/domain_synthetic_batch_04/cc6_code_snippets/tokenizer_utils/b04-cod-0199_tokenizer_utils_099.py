...
@dataclass
class TokenizerUtilsExample099:
    topic_id: str = "B04-COD-0199"
...
def build_tokenizer_utils_example_099(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "tokenizer_utils" / "B04-COD-0199.json")


def main() -> None:
    example = TokenizerUtilsExample099()
    stats = build_tokenizer_utils_example_099([99, 100, 101, 102])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0199"
