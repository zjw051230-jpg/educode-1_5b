...
"""Synthetic teaching example for tokenizer encode/decode helper with focus on deterministic split helper."""
...
@dataclass
class TokenizerUtilsExample100:
    topic_id: str = "B04-COD-0200"
...
def build_tokenizer_utils_example_100(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "tokenizer_utils" / "B04-COD-0200.json")


def main() -> None:
    example = TokenizerUtilsExample100()
    stats = build_tokenizer_utils_example_100([100, 101, 102, 103])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0200"
