...
@dataclass
class ValidationLoopExample039:
    topic_id: str = "B04-COD-0739"
...
def build_validation_loop_example_039(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "validation_loop" / "B04-COD-0739.json")


def main() -> None:
    example = ValidationLoopExample039()
    stats = build_validation_loop_example_039([39, 40, 41, 42])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0739"
