...
@dataclass
class ValidationLoopExample058:
    topic_id: str = "B04-COD-0758"
...
def build_validation_loop_example_058(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "validation_loop" / "B04-COD-0758.json")


def main() -> None:
    example = ValidationLoopExample058()
    stats = build_validation_loop_example_058([58, 59, 60, 61])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0758"
