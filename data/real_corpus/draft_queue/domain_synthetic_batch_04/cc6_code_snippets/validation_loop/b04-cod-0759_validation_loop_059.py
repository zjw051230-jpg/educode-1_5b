...
@dataclass
class ValidationLoopExample059:
    topic_id: str = "B04-COD-0759"
...
def build_validation_loop_example_059(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "validation_loop" / "B04-COD-0759.json")


def main() -> None:
    example = ValidationLoopExample059()
    stats = build_validation_loop_example_059([59, 60, 61, 62])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0759"
