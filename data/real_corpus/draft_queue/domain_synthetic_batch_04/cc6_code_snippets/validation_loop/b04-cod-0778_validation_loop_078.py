...
@dataclass
class ValidationLoopExample078:
    topic_id: str = "B04-COD-0778"
...
def build_validation_loop_example_078(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "validation_loop" / "B04-COD-0778.json")


def main() -> None:
    example = ValidationLoopExample078()
    stats = build_validation_loop_example_078([78, 79, 80, 81])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0778"
