...
@dataclass
class ValidationLoopExample092:
    topic_id: str = "B04-COD-0792"
...
def build_validation_loop_example_092(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "validation_loop" / "B04-COD-0792.json")


def main() -> None:
    example = ValidationLoopExample092()
    stats = build_validation_loop_example_092([92, 93, 94, 95])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0792"
