...
@dataclass
class ValidationLoopExample094:
    topic_id: str = "B04-COD-0794"
...
def build_validation_loop_example_094(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "validation_loop" / "B04-COD-0794.json")


def main() -> None:
    example = ValidationLoopExample094()
    stats = build_validation_loop_example_094([94, 95, 96, 97])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0794"
