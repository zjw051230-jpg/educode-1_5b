...
@dataclass
class ValidationLoopExample096:
    topic_id: str = "B04-COD-0796"
...
def build_validation_loop_example_096(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "validation_loop" / "B04-COD-0796.json")


def main() -> None:
    example = ValidationLoopExample096()
    stats = build_validation_loop_example_096([96, 97, 98, 99])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0796"
