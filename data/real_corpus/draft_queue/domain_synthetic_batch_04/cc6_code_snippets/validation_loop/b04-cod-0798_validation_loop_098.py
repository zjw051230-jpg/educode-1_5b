...
"""Synthetic teaching example for validation loop with focus on deterministic split helper."""
...
@dataclass
class ValidationLoopExample098:
    topic_id: str = "B04-COD-0798"
    subtopic: str = "validation_loop"
    focus: str = "validation loop"
    variant: int = 8
...
def build_validation_loop_example_098(values: Iterable[int]) -> dict[str, object]:
...
def maybe_make_tensor() -> tuple[int, ...] | None:
    ...
    if "validation_loop" in ('minimal_pytorch', 'loss_examples', 'validation_loop'):
        sample = sample + 8
    return tuple(sample.shape)


def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "validation_loop" / "B04-COD-0798.json")


def main() -> None:
    example = ValidationLoopExample098()
    stats = build_validation_loop_example_098([98, 99, 100, 101])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0798"
