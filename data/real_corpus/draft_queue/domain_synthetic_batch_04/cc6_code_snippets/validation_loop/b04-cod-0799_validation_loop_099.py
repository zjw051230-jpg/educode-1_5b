...
"""Synthetic teaching example for validation loop with focus on validation loop."""
...
@dataclass
class ValidationLoopExample099:
    topic_id: str = "B04-COD-0799"
...
def build_validation_loop_example_099(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "validation_loop" / "B04-COD-0799.json")


def main() -> None:
    example = ValidationLoopExample099()
    stats = build_validation_loop_example_099([99, 100, 101, 102])
    payload = {
        ...
        "extra_focus": "validation loop",
    }
    encoded = json.dumps(payload, ensure_ascii=False, sort_keys=True)
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0799"
