...
"""Synthetic teaching example for validation loop with focus on finite loss check."""
...
@dataclass
class ValidationLoopExample100:
    topic_id: str = "B04-COD-0800"
...
def build_validation_loop_example_100(values: Iterable[int]) -> dict[str, object]:
...
def build_preview_path(repo_root: str = "/synthetic/repo") -> str:
    return str(Path(repo_root) / "data" / "draft_queue" / "domain_synthetic_batch_04" / "validation_loop" / "B04-COD-0800.json")


def main() -> None:
    example = ValidationLoopExample100()
    stats = build_validation_loop_example_100([100, 101, 102, 103])
    payload = {
    ...
    print(encoded)
    assert payload["meta"]["topic_id"] == "B04-COD-0800"
