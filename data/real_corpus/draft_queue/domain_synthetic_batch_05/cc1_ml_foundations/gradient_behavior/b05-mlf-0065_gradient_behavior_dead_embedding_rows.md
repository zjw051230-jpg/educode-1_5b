...
The learning objective is specific: Show how dead embedding rows can hide behind healthy-looking global gradient norms.
...
Shape trace:
- logits: [batch=2, time=6, vocab=16]
- targets: [batch=2, time=6]
- active mask: [batch=2, time=6] with one dropped suffix position

## Signals that can mislead
The first trap in dead embedding rows is to turn one scalar into a universal rule.
