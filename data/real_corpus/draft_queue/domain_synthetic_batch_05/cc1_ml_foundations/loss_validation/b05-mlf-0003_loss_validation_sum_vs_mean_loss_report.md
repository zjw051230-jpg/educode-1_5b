...
# Sum Versus Mean Loss Report

Instead of asking what loss means in general, this file asks why one batch report felt wrong.
The learning objective is specific: Compare summed loss and mean loss so the reader can explain why two runs look different despite identical token errors.
...
- focus: sum_versus_mean_loss_report
...
The first trap in sum versus mean loss report is to turn one scalar into a universal rule.
