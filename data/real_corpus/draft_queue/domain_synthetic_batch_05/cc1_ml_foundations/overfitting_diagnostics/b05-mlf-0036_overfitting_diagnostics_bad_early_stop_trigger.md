...
# Bad Early-Stop Trigger

The easiest way to teach overfitting is to show how a team could misread it in real time.
The learning objective is specific: Explain how a poor early-stop trigger can either freeze an underfit model or preserve an overfit one.
This note is written as a failure postmortem and uses a config snippet as the concrete anchor.

## Incident setup
- topic_id: B05-MLF-0036
- subdirectory: overfitting_diagnostics
- focus: bad_early_stop_trigger
...
The first trap in bad early-stop trigger is to turn one scalar into a universal rule.
