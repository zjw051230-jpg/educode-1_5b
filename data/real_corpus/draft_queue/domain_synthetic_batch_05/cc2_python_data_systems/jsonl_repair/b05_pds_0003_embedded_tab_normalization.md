...
# Embedded Tab Normalization

## What the learner should leave with
explain when embedded tabs should be preserved versus normalized before JSONL parsing.

## Broken case on the table
This note focuses on a before/after comparison inside `jsonl_repair`.
The example is intentionally small so the repair rule stays visible instead of hiding inside pipeline boilerplate.

## Concrete sample
```text
topic_id: B05-PDS-0003
broken_focus: embedded_tab_normalization
...
## Distinctive detail
The learning objective is not “understand jsonl_repair broadly”; it is to explain when embedded tabs should be preserved versus normalized before JSONL parsing.
