...
# Multi Line Field Rejection

## What the learner should leave with
diagnose why a multi-line field breaks one-object-per-line assumptions.

## Broken case on the table
This note focuses on a failure scenario inside `jsonl_repair`.
The example is intentionally small so the repair rule stays visible instead of hiding inside pipeline boilerplate.

## Concrete sample
```text
topic_id: B05-PDS-0005
broken_focus: multi_line_field_rejection
...
## Distinctive detail
The learning objective is not “understand jsonl_repair broadly”; it is to diagnose why a multi-line field breaks one-object-per-line assumptions.
