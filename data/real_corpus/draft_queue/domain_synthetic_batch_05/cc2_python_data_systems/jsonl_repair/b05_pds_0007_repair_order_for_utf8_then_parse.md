...
# Repair Order For Utf8 Then Parse

## What the learner should leave with
choose a repair order for utf-8 cleanup, whitespace trimming, and parsing.

## Broken case on the table
This note focuses on a repair checklist inside `jsonl_repair`.
The example is intentionally small so the repair rule stays visible instead of hiding inside pipeline boilerplate.

## Concrete sample
```text
topic_id: B05-PDS-0007
broken_focus: repair_order_for_utf8_then_parse
...
## Distinctive detail
The learning objective is not “understand jsonl_repair broadly”; it is to choose a repair order for utf-8 cleanup, whitespace trimming, and parsing.
