...
# Reader Preview Budget

## What the learner should leave with
interpret a preview window that stops before the first broken line.

## Broken case on the table
This note focuses on a small pseudo-run log inside `jsonl_repair`.
The example is intentionally small so the repair rule stays visible instead of hiding inside pipeline boilerplate.

## Concrete sample
```text
topic_id: B05-PDS-0013
broken_focus: reader_preview_budget
...
## Distinctive detail
The learning objective is not “understand jsonl_repair broadly”; it is to interpret a preview window that stops before the first broken line.
