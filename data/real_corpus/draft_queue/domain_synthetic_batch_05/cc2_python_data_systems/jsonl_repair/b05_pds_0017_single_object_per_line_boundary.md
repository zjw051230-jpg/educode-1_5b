...
# Single Object Per Line Boundary

## What the learner should leave with
teach why a wrapped pretty-printed object should be rejected in JSONL.

## Broken case on the table
This note focuses on a broken jsonl case inside `jsonl_repair`.
The example is intentionally small so the repair rule stays visible instead of hiding inside pipeline boilerplate.

## Concrete sample
```text
topic_id: B05-PDS-0017
broken_focus: single_object_per_line_boundary
...
## Distinctive detail
The learning objective is not “understand jsonl_repair broadly”; it is to teach why a wrapped pretty-printed object should be rejected in JSONL.
