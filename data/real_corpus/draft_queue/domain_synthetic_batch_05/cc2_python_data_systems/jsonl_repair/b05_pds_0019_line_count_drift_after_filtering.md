...
# Line Count Drift After Filtering

## What the learner should leave with
explain line-count drift after removing empty tail lines from a sample file.

## Broken case on the table
This note focuses on a numeric toy example inside `jsonl_repair`.
The example is intentionally small so the repair rule stays visible instead of hiding inside pipeline boilerplate.

## Concrete sample
```text
topic_id: B05-PDS-0019
broken_focus: line_count_drift_after_filtering
...
## Distinctive detail
The learning objective is not “understand jsonl_repair broadly”; it is to explain line-count drift after removing empty tail lines from a sample file.
