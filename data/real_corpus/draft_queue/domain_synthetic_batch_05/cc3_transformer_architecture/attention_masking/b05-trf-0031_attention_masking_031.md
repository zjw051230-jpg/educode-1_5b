...
## Opening question
What should a reviewer inspect first when a mask matrix example looks plausible but one dimension is silently wrong?

## Learning objective
explain how causal and padding masks combine at runtime; file 0031 uses a before/after comparison to keep the lesson specific.
...
## Shape trace
1. start hidden: `(2, 4, 20)`
2. project or inspect the mask matrix path before any silent broadcast
3. compare the expected inner width against the observed trace
4. mark the first line where the invariant breaks

## Why this case matters
The file avoids generic transformer narration by staying inside one concrete mask matrix example.
