...
# Why absolute positions show up as shape bugs (position embedding table)

## Failure scenario
A teammate reports that the position embedding table branch is “probably fine” because the outer tensor shape still matches.
...
### Better response
Ask for one before/after comparison, one explicit tuple, and one statement of the intended invariant.
