...
# First Run Compile Penalty

A runtime reviewer usually gets this topic wrong when the title is read without the artifacts that produced it.

**Learning objective:** Explain why the first smoke pass shows worse step time than the second pass with the same config.
...
## Example packet
```text
step=12481 train_tokens_per_sec=129610
step=12481 max_memory_allocated_gb=44.1
step=12481 max_memory_reserved_gb=66.7
step=12482 validation_start seq_len=4608 microbatch=8
step=12483 validation_end val_loss=1.890
```
