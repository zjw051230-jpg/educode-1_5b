...
# Attention Kernel Fallback Signature

This draft treats attention kernel fallback signature as a bounded incident reconstruction problem rather than a generic lesson.

**Learning objective:** Recognize the log signature of a slow attention kernel fallback during an otherwise healthy smoke run.
...
## Observed packet
```text
phase=train step=12484 tok/s=130040
phase=eval  step=12485 val_loss=1.820
phase=eval  step=12485 reserved_gb=62.4
phase=save  step=12486 save_ms=514
summary.json latest_step=12486 best_step=12465
```
...
## Concrete anchor
This file uses **metrics interpretation** rather than a generic note. The failure is described with bounded numbers: allocated `37.5` GB, reserved `62.4` GB, peak `66.0` GB, sequence length `2048`.
