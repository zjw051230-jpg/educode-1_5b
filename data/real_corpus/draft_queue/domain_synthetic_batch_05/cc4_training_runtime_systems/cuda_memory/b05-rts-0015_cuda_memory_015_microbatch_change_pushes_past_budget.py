...
def build_packet_microbatch_change_pushes_past_budget() -> dict:
    return {
        "topic_id": "B05-RTS-0015",
        "title": "Microbatch Pushes Past Budget",
        "learning_objective": "Use a numeric budget sheet to explain why increasing microbatch from 4 to 5 causes OOM at seq_len 4096.",
        "writing_form": "explainer note",
        "concrete_anchor": "failure scenario",
        "artifact_focus": "microbatch/seq_len memory worksheet",
        "step": 12075,
        "seq_len": 4608,
        "max_memory_allocated_gb": 44.4,
        "max_memory_reserved_gb": 66.7,
        "peak_reserved_gb": 69.5,
        "train_tokens_per_sec": 123650,
        "val_loss": 2.12,
        "weighted_val_loss": 2.089,
        "latest_step": 12077,
        "best_step": 12051,
        "optimizer_step": 12075,
        "event_rows": [
            {"event": "train_step", "step": 12075, "tokens_per_sec": 123650},
            {"event": "eval_end", "step": 12076, "val_loss": 2.12, "samples": 512},
            {"event": "checkpoint_save", "step": 12077, "duration_ms": 435},
        ],
    }
...
def diagnose_microbatch_change_pushes_past_budget(packet: dict) -> DiagnosticResult:
    evidence: list[str] = []
    suspicious = False
...
def main() -> None:
    packet = build_packet_microbatch_change_pushes_past_budget()
    result = diagnose_microbatch_change_pushes_past_budget(packet)
