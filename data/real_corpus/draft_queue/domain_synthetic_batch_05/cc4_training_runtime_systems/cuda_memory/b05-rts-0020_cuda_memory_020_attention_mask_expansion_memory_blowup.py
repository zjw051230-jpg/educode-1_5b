...
def build_packet_attention_mask_expansion_memory_blowup() -> dict:
    return {
        "topic_id": "B05-RTS-0020",
        "title": "Attention Mask Expansion Cost",
        "learning_objective": "Trace how an attention mask expansion from [8,4096] to [8,1,4096,4096] changes memory expectations.",
        "writing_form": "debugging diary",
        "concrete_anchor": "mini code trace",
        "artifact_focus": "tensor shapes + estimated bytes",
        "step": 12100,
        "seq_len": 2048,
        "max_memory_allocated_gb": 40.8,
        "max_memory_reserved_gb": 61.0,
        "peak_reserved_gb": 63.8,
        "train_tokens_per_sec": 126200,
        "val_loss": 1.72,
        "weighted_val_loss": 1.689,
        "latest_step": 12102,
        "best_step": 12076,
        "optimizer_step": 12100,
        "event_rows": [
            {"event": "train_step", "step": 12100, "tokens_per_sec": 126200},
            {"event": "eval_end", "step": 12101, "val_loss": 1.72, "samples": 512},
            {"event": "checkpoint_save", "step": 12102, "duration_ms": 450},
        ],
    }
...
def diagnose_attention_mask_expansion_memory_blowup(packet: dict) -> DiagnosticResult:
    evidence: list[str] = []
    suspicious = False
...
def main() -> None:
    packet = build_packet_attention_mask_expansion_memory_blowup()
    result = diagnose_attention_mask_expansion_memory_blowup(packet)
