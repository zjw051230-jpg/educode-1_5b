...
def build_packet_tokens_seen_counter_disagrees_with_global_step() -> dict:
    return {
        "topic_id": "B05-RTS-0033",
        "title": "Tokens Seen Counter Disagrees",
        "learning_objective": "Reconcile a token counter that implies 1.6B tokens seen while the step count suggests only 1.4B.",
        "writing_form": "checklist",
        "concrete_anchor": "numeric toy example",
        "artifact_focus": "metrics rows + config math",
        "step": 12165,
        "seq_len": 3072,
        "max_memory_allocated_gb": 37.2,
        "max_memory_reserved_gb": 59.5,
        "peak_reserved_gb": 62.3,
        "train_tokens_per_sec": 132830,
        "val_loss": 1.8,
        "weighted_val_loss": 1.769,
        "latest_step": 12167,
        "best_step": 12141,
        "optimizer_step": 12162,
        "event_rows": [
            {"event": "train_step", "step": 12165, "tokens_per_sec": 132830},
            {"event": "eval_end", "step": 12166, "val_loss": 1.8, "samples": 512},
            {"event": "checkpoint_save", "step": 12167, "duration_ms": 489},
        ],
    }
...
def diagnose_tokens_seen_counter_disagrees_with_global_step(packet: dict) -> DiagnosticResult:
    evidence: list[str] = []
    suspicious = False
...
def main() -> None:
    packet = build_packet_tokens_seen_counter_disagrees_with_global_step()
    result = diagnose_tokens_seen_counter_disagrees_with_global_step(packet)
