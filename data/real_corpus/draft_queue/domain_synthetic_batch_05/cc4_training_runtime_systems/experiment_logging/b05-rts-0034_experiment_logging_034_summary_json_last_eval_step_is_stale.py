...
def build_packet_summary_json_last_eval_step_is_stale() -> dict:
    return {
        "topic_id": "B05-RTS-0034",
        "title": "Last Eval Step Is Stale",
        "learning_objective": "Show how last_eval_step can lag behind the latest eval_end event in metrics.jsonl.",
        "writing_form": "failure analysis",
        "concrete_anchor": "config snippet",
        "artifact_focus": "summary.json + eval_end rows",
        "step": 12170,
        "seq_len": 4096,
        "max_memory_allocated_gb": 38.4,
        "max_memory_reserved_gb": 62.8,
        "peak_reserved_gb": 65.6,
        "train_tokens_per_sec": 133340,
        "val_loss": 1.72,
        "weighted_val_loss": 1.689,
        "latest_step": 12172,
        "best_step": 12146,
        "optimizer_step": 12166,
        "event_rows": [
            {"event": "train_step", "step": 12170, "tokens_per_sec": 133340},
            {"event": "eval_end", "step": 12171, "val_loss": 1.72, "samples": 512},
            {"event": "checkpoint_save", "step": 12172, "duration_ms": 492},
        ],
    }
...
def diagnose_summary_json_last_eval_step_is_stale(packet: dict) -> DiagnosticResult:
    evidence: list[str] = []
    suspicious = False
...
def main() -> None:
    packet = build_packet_summary_json_last_eval_step_is_stale()
    result = diagnose_summary_json_last_eval_step_is_stale(packet)
