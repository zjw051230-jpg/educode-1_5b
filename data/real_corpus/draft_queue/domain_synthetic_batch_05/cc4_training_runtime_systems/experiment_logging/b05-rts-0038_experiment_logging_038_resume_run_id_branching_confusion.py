...
def build_packet_resume_run_id_branching_confusion() -> dict:
    return {
        "topic_id": "B05-RTS-0038",
        "title": "Resume Run Id Branching",
        "learning_objective": "Explain how two resumed runs sharing a prefix create misleading merged dashboards.",
        "writing_form": "failure analysis",
        "concrete_anchor": "debugging transcript",
        "artifact_focus": "run_id examples + dashboard rows",
        "step": 12190,
        "seq_len": 4096,
        "max_memory_allocated_gb": 43.2,
        "max_memory_reserved_gb": 63.4,
        "peak_reserved_gb": 66.2,
        "train_tokens_per_sec": 135380,
        "val_loss": 1.96,
        "weighted_val_loss": 1.929,
        "latest_step": 12192,
        "best_step": 12166,
        "optimizer_step": 12187,
        "event_rows": [
            {"event": "train_step", "step": 12190, "tokens_per_sec": 135380},
            {"event": "eval_end", "step": 12191, "val_loss": 1.96, "samples": 512},
            {"event": "checkpoint_save", "step": 12192, "duration_ms": 504},
        ],
    }
...
def diagnose_resume_run_id_branching_confusion(packet: dict) -> DiagnosticResult:
    evidence: list[str] = []
    suspicious = False
...
def main() -> None:
    packet = build_packet_resume_run_id_branching_confusion()
    result = diagnose_resume_run_id_branching_confusion(packet)
