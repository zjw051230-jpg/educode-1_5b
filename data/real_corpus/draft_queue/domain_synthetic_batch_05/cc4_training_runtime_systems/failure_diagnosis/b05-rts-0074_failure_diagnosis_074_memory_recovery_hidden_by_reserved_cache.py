...
def build_packet_memory_recovery_hidden_by_reserved_cache() -> dict:
    return {
        "topic_id": "B05-RTS-0074",
        "title": "Memory Recovery Hidden By Reserved Cache",
        "learning_objective": "Interpret an incident where allocated memory recovers after eval but reserved memory masks that recovery in the headline chart.",
        "writing_form": "comparison table",
        "concrete_anchor": "debugging transcript",
        "artifact_focus": "allocated vs reserved chart table",
        "step": 12370,
        "seq_len": 4096,
        "max_memory_allocated_gb": 38.4,
        "max_memory_reserved_gb": 58.6,
        "peak_reserved_gb": 61.4,
        "train_tokens_per_sec": 153740,
        "val_loss": 1.88,
        "weighted_val_loss": 1.849,
        "latest_step": 12372,
        "best_step": 12346,
        "optimizer_step": 12366,
        "event_rows": [
            {"event": "train_step", "step": 12370, "tokens_per_sec": 153740},
            {"event": "eval_end", "step": 12371, "val_loss": 1.88, "samples": 512},
            {"event": "checkpoint_save", "step": 12372, "duration_ms": 612},
        ],
    }
...
def diagnose_memory_recovery_hidden_by_reserved_cache(packet: dict) -> DiagnosticResult:
    evidence: list[str] = []
    suspicious = False
...
def main() -> None:
    packet = build_packet_memory_recovery_hidden_by_reserved_cache()
    result = diagnose_memory_recovery_hidden_by_reserved_cache(packet)
