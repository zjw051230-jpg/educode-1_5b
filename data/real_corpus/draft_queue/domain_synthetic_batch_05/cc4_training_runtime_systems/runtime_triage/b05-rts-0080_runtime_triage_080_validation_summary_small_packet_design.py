...
def build_packet_validation_summary_small_packet_design() -> dict:
    return {
        "topic_id": "B05-RTS-0080",
        "title": "Validation Packet Design",
        "learning_objective": "Choose the minimum runtime fields needed in a bounded validation packet to preserve diagnostic value.",
        "writing_form": "comparison table",
        "concrete_anchor": "mini code trace",
        "artifact_focus": "field shortlist with rationale",
        "step": 12400,
        "seq_len": 2048,
        "max_memory_allocated_gb": 36.0,
        "max_memory_reserved_gb": 56.2,
        "peak_reserved_gb": 59.0,
        "train_tokens_per_sec": 156800,
        "val_loss": 1.96,
        "weighted_val_loss": 1.929,
        "latest_step": 12402,
        "best_step": 12376,
        "optimizer_step": 12400,
        "event_rows": [
            {"event": "train_step", "step": 12400, "tokens_per_sec": 156800},
            {"event": "eval_end", "step": 12401, "val_loss": 1.96, "samples": 512},
            {"event": "checkpoint_save", "step": 12402, "duration_ms": 630},
        ],
    }
...
def diagnose_validation_summary_small_packet_design(packet: dict) -> DiagnosticResult:
    evidence: list[str] = []
    suspicious = False
...
def main() -> None:
    packet = build_packet_validation_summary_small_packet_design()
    result = diagnose_validation_summary_small_packet_design(packet)
