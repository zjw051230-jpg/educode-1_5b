...
def build_packet_config_override_audit_for_runtime_regression() -> dict:
    return {
        "topic_id": "B05-RTS-0094",
        "title": "Config Override Audit For Regression",
        "learning_objective": "Audit a small config override set to determine which changed field most likely introduced a runtime regression.",
        "writing_form": "metric interpretation",
        "concrete_anchor": "config snippet",
        "artifact_focus": "config diff + event timeline",
        "step": 12470,
        "seq_len": 4096,
        "max_memory_allocated_gb": 43.2,
        "max_memory_reserved_gb": 67.6,
        "peak_reserved_gb": 70.4,
        "train_tokens_per_sec": 163940,
        "val_loss": 1.96,
        "weighted_val_loss": 1.929,
        "latest_step": 12472,
        "best_step": 12446,
        "optimizer_step": 12466,
        "event_rows": [
            {"event": "train_step", "step": 12470, "tokens_per_sec": 163940},
            {"event": "eval_end", "step": 12471, "val_loss": 1.96, "samples": 512},
            {"event": "checkpoint_save", "step": 12472, "duration_ms": 672},
        ],
    }
...
def diagnose_config_override_audit_for_runtime_regression(packet: dict) -> DiagnosticResult:
    evidence: list[str] = []
    suspicious = False
...
def main() -> None:
    packet = build_packet_config_override_audit_for_runtime_regression()
    result = diagnose_config_override_audit_for_runtime_regression(packet)
