...
def build_packet_promotion_blocker_severity_ladder() -> dict:
    return {
        "topic_id": "B05-RTS-0096",
        "title": "Promotion Blocker Severity Ladder",
        "learning_objective": "Rank runtime promotion blockers by severity using a ladder tied to missing artifacts, stale fields, and boundary anomalies.",
        "writing_form": "metric interpretation",
        "concrete_anchor": "metrics interpretation",
        "artifact_focus": "severity matrix",
        "step": 12480,
        "seq_len": 2048,
        "max_memory_allocated_gb": 36.0,
        "max_memory_reserved_gb": 52.0,
        "peak_reserved_gb": 54.8,
        "train_tokens_per_sec": 164960,
        "val_loss": 1.8,
        "weighted_val_loss": 1.769,
        "latest_step": 12482,
        "best_step": 12456,
        "optimizer_step": 12479,
        "event_rows": [
            {"event": "train_step", "step": 12480, "tokens_per_sec": 164960},
            {"event": "eval_end", "step": 12481, "val_loss": 1.8, "samples": 512},
            {"event": "checkpoint_save", "step": 12482, "duration_ms": 678},
        ],
    }
...
def diagnose_promotion_blocker_severity_ladder(packet: dict) -> DiagnosticResult:
    evidence: list[str] = []
    suspicious = False
...
def main() -> None:
    packet = build_packet_promotion_blocker_severity_ladder()
    result = diagnose_promotion_blocker_severity_ladder(packet)
