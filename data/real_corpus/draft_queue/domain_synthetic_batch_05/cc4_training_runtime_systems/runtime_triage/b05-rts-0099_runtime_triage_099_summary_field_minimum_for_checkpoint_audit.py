...
def build_packet_summary_field_minimum_for_checkpoint_audit() -> dict:
    return {
        "topic_id": "B05-RTS-0099",
        "title": "Summary Fields For Checkpoint Audit",
        "learning_objective": "Decide which summary.json fields are the minimum viable set for auditing checkpoint quality without reopening the whole run.",
        "writing_form": "config review",
        "concrete_anchor": "failure scenario",
        "artifact_focus": "summary field shortlist",
        "step": 12495,
        "seq_len": 4608,
        "max_memory_allocated_gb": 39.6,
        "max_memory_reserved_gb": 61.9,
        "peak_reserved_gb": 64.7,
        "train_tokens_per_sec": 166490,
        "val_loss": 2.12,
        "weighted_val_loss": 2.089,
        "latest_step": 12497,
        "best_step": 12471,
        "optimizer_step": 12491,
        "event_rows": [
            {"event": "train_step", "step": 12495, "tokens_per_sec": 166490},
            {"event": "eval_end", "step": 12496, "val_loss": 2.12, "samples": 512},
            {"event": "checkpoint_save", "step": 12497, "duration_ms": 687},
        ],
    }
...
def diagnose_summary_field_minimum_for_checkpoint_audit(packet: dict) -> DiagnosticResult:
    evidence: list[str] = []
    suspicious = False
...
def main() -> None:
    packet = build_packet_summary_field_minimum_for_checkpoint_audit()
    result = diagnose_summary_field_minimum_for_checkpoint_audit(packet)
