...
def build_packet_gradient_accumulation_changes_reporting_unit() -> dict:
    return {
        "topic_id": "B05-RTS-0058",
        "title": "Grad Accum Changes Reporting Unit",
        "learning_objective": "Compare tokens/sec and optimizer_steps/sec before and after changing grad_accum from 1 to 8.",
        "writing_form": "Q&A",
        "concrete_anchor": "config snippet",
        "artifact_focus": "before/after metrics table",
        "step": 12290,
        "seq_len": 4096,
        "max_memory_allocated_gb": 38.4,
        "max_memory_reserved_gb": 62.8,
        "peak_reserved_gb": 65.6,
        "train_tokens_per_sec": 145580,
        "val_loss": 2.04,
        "weighted_val_loss": 2.009,
        "latest_step": 12292,
        "best_step": 12266,
        "optimizer_step": 12287,
        "event_rows": [
            {"event": "train_step", "step": 12290, "tokens_per_sec": 145580},
            {"event": "eval_end", "step": 12291, "val_loss": 2.04, "samples": 512},
            {"event": "checkpoint_save", "step": 12292, "duration_ms": 564},
        ],
    }
...
def diagnose_gradient_accumulation_changes_reporting_unit(packet: dict) -> DiagnosticResult:
    evidence: list[str] = []
    suspicious = False
...
def main() -> None:
    packet = build_packet_gradient_accumulation_changes_reporting_unit()
    result = diagnose_gradient_accumulation_changes_reporting_unit(packet)
