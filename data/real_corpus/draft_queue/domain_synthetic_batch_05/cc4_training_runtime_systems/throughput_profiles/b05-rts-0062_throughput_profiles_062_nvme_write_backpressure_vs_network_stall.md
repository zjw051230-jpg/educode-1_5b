...
# Nvme Backpressure Or Network Stall

A runtime reviewer usually gets this topic wrong when the title is read without the artifacts that produced it.

**Learning objective:** Differentiate local artifact write backpressure from a network log sink stall using timing clues.
...
|---|---|---|---|
| latest step | 12587 | 12588 | B if the save event is confirmed |
| val_loss | 2.020 | 1.960 | depends on denominator visibility |
| reserved_gb | 62.7 | 66.7 | B if eval spike is real |
| event typing | missing | explicit eval_end/save | B |
| throughput | 139260 | 144660 | compare with boundary timing |

## Concrete anchor
This file uses **debugging transcript**. The table is not decorative; each cell is a claim that must map back to a concrete artifact.

## Artifact packet
```text
metrics.jsonl: event=eval_end step=12587 val_loss=1.960
metrics.jsonl: event=checkpoint_save step=12588 duration_ms=648
summary.json: latest_step=12588
memory.log: reserved_gb=66.7
```
...
- compare save duration with sink flush latency in the same step window
- inspect whether the slowdown starts before or after the artifact write boundary
