...
# Mixed Precision Eval Overflow

Instead of explaining mixed precision eval overflow in the abstract, this file starts from a concrete artifact packet.

**Learning objective:** Interpret validation rows that suddenly become nan after a single overflow event in mixed precision.

## Q1. What is the bounded runtime scene?
A run near step `12551` emitted `eval log + scaler event` and one neighboring event sequence. The scene is deliberately small so the diagnosis stays specific.
...
- peak_gb = 60.2
- seq_len = 4096
- tokens_per_sec = 139500

## Q3. What does the packet look like?
```text
step=12550 event=train_step
step=12551 event=eval_end val_loss=1.960
step=12551 event=summary_write best_step=12521
step=12552 event=checkpoint_save latest_step=12552
```
