...
# learning_objective: Implement window slicing for next-token prediction.
...
TOPIC_ID = 'B05-COD-0047'
TITLE = 'Slice next-token windows'
...
def topic_action(samples: list[list[int]]) -> object:
    tokens = samples[0]
    window = 4
    return [tokens[i:i + window] for i in range(0, len(tokens) - window + 1)]
