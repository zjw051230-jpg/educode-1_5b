...
# learning_objective: Validate that a checkpoint payload includes the keys needed for resume.
...
TOPIC_ID = 'B05-COD-0027'
TITLE = 'Check required checkpoint metadata keys'
...
def topic_action(payload: dict[str, object]) -> object:
    return [key for key in ('step', 'metadata', 'config', 'path') if key not in payload]
