...
# learning_objective: Produce an explicit resume verdict from metadata evidence.
...
TOPIC_ID = 'B05-COD-0029'
TITLE = 'Build a resume safety verdict'
...
def topic_action(payload: dict[str, object]) -> object:
    checks = {'has_step': 'step' in payload, 'has_run_name': 'run_name' in payload['metadata'], 'has_path': 'path' in payload}
    checks['resume_safe'] = all(checks.values())
    return checks
