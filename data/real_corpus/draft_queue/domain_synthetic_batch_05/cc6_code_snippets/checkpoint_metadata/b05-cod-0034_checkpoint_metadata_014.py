...
# learning_objective: Reject empty or overly long operator notes.
...
TOPIC_ID = 'B05-COD-0034'
TITLE = 'Validate checkpoint notes length and emptiness'
...
def topic_action(payload: dict[str, object]) -> object:
    notes = str(payload['metadata'].get('notes', ''))
    return {'non_empty': bool(notes.strip()), 'short_enough': len(notes) <= 80}
