...
# learning_objective: Explain differences between two checkpoint metadata payloads.
...
TOPIC_ID = 'B05-COD-0038'
TITLE = 'Compare two checkpoint metadata payloads'
...
def topic_action(payload: dict[str, object]) -> object:
    other = {'metadata': {'run_name': 'cc6-demo', 'tokenizer_hash': 'tok-b2'}}
    keys = sorted(set(payload['metadata']) | set(other['metadata']))
    return {k: (payload['metadata'].get(k), other['metadata'].get(k)) for k in keys if payload['metadata'].get(k) != other['metadata'].get(k)}
