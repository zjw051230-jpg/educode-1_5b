...
# learning_objective: Report path-related config problems in one place.
...
TOPIC_ID = 'B05-COD-0070'
TITLE = 'Collect path validation errors'
...
def topic_action(config: dict[str, object]) -> object:
    ...
    if not config['paths']['tokenizer_path']:
        errors.append('tokenizer_path missing')
    if not config['paths']['resume_path']:
        errors.append('resume_path missing')
    return errors
