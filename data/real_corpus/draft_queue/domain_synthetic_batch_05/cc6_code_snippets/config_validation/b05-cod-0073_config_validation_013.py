...
# learning_objective: Require tokenizer paths only when a matching mode is enabled.
...
TOPIC_ID = 'B05-COD-0073'
TITLE = 'Require tokenizer path when text mode is enabled'
...
def topic_action(config: dict[str, object]) -> object:
    return (not config.get('mode', {}).get('text_mode', True)) or bool(config['paths']['tokenizer_path'])
