...
# learning_objective: Turn raw validation findings into operator-facing messages.
...
TOPIC_ID = 'B05-COD-0074'
TITLE = 'Explain config errors as operator messages'
...
def topic_action(config: dict[str, object]) -> object:
    ...
    if config['training']['precision'] not in {'fp32', 'bf16'}:
        messages.append('precision must be fp32 or bf16')
    if int(config['training']['micro_batch_size']) <= 0:
        messages.append('micro_batch_size must be positive')
    return messages
