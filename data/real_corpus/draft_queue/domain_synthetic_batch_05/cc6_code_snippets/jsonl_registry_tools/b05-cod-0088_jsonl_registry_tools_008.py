...
# learning_objective: Validate that required registry fields are present.
...
TOPIC_ID = 'B05-COD-0088'
TITLE = 'Validate required registry fields'
...
def topic_action(text: str) -> object:
    ...
    for row in parse_rows(text):
        for field in ('topic_id', 'file_type'):
            if field not in row:
                errors.append(fline {row['_line_number']}: missing {field})
    return errors
