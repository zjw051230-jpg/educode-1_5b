...
# learning_objective: Normalize file-type spellings before aggregation.
...
TOPIC_ID = 'B05-COD-0089'
TITLE = 'Normalize file type labels'
...
def topic_action(text: str) -> object:
    mapping = {'markdown': 'markdown', 'py': 'python'}
    return [mapping.get(str(row['file_type']).strip().lower(), str(row['file_type']).strip().lower()) for row in parse_rows(text)]
