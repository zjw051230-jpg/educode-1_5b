...
# learning_objective: Summarize manifest rows by worker and file type.
...
TOPIC_ID = 'B05-COD-0092'
TITLE = 'Summarize rows by worker and file type'
...
def topic_action(text: str) -> object:
    ...
    rows.append({'worker_id': 'CC-6', 'file_type': 'python'})
    grouped: defaultdict[str, Counter[str]] = defaultdict(Counter)
    for row in rows:
        grouped[str(row.get('worker_id', 'CC-6'))][str(row['file_type']).lower()] += 1
    return {worker: dict(counter) for worker, counter in sorted(grouped.items())}
