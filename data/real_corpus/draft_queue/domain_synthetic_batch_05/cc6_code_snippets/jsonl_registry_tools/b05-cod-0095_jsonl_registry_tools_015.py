...
# learning_objective: Build a small report row from registry data.
...
TOPIC_ID = 'B05-COD-0095'
TITLE = 'Project registry rows into a small report'
...
def topic_action(text: str) -> object:
    return [{'topic_id': row['topic_id'], 'file_type': row['file_type'], 'has_title': bool(str(row.get('title', '')).strip())} for row in parse_rows(text)]
