...
# learning_objective: Filter out rows whose titles are empty.
...
TOPIC_ID = 'B05-COD-0096'
TITLE = 'Filter rows with missing titles'
...
def topic_action(text: str) -> object:
    return [row['topic_id'] for row in parse_rows(text) if not str(row.get('title', '')).strip()]
