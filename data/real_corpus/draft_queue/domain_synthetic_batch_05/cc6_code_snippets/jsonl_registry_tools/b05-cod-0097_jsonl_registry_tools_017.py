...
# learning_objective: Detect duplicate filenames within a subdirectory.
...
TOPIC_ID = 'B05-COD-0097'
TITLE = 'Detect duplicate filenames inside a subdirectory'
...
def topic_action(text: str) -> object:
    ...
    rows.append({'subdirectory': 'jsonl_registry_tools', 'filename': 'dup.py'})
    rows.append({'subdirectory': 'jsonl_registry_tools', 'filename': 'dup.py'})
    seen = Counter((row['subdirectory'], row.get('filename', '')) for row in rows)
    return [filename for (_, filename), count in seen.items() if filename and count > 1]
