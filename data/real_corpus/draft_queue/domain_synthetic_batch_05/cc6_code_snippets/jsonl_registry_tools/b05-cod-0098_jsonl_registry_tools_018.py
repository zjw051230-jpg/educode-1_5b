...
# learning_objective: Count approval states across rows.
...
TOPIC_ID = 'B05-COD-0098'
TITLE = 'Count approved vs draft rows'
...
def topic_action(text: str) -> object:
    ...
    rows[0]['approved_for_training'] = False
    rows[1]['approved_for_training'] = True
    return {'approved': sum(bool(row['approved_for_training']) for row in rows), 'draft_only': sum(not bool(row['approved_for_training']) for row in rows)}
