...
# learning_objective: Verify that paths stay under the expected worker prefix.
...
TOPIC_ID = 'B05-COD-0099'
TITLE = 'Check manifest path prefixes'
...
def topic_action(text: str) -> object:
    prefix = 'data/real_corpus/draft_queue/domain_synthetic_batch_05/cc6_code_snippets/'
    rows = [{'relative_path': prefix + 'jsonl_registry_tools/a.py'}, {'relative_path': 'other/path/b.py'}]
    return [row['relative_path'] for row in rows if not row['relative_path'].startswith(prefix)]
