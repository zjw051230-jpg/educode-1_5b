...
# learning_objective: Collect path groups per category to make spot checks easier.
...
TOPIC_ID = 'B05-COD-0008'
TITLE = 'Group relative paths by category'
...
def topic_action(rows: list[dict[str, object]]) -> object:
    ...
    for row in rows:
        grouped[str(row['source_category'])].append(str(row['relative_path']))
