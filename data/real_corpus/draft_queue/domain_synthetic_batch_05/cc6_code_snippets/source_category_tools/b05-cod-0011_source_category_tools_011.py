...
# learning_objective: Reject unknown labels with an explicit alert list.
...
TOPIC_ID = 'B05-COD-0011'
TITLE = 'Detect unknown source categories'
...
def topic_action(rows: list[dict[str, object]]) -> object:
    allowed = {'synthetic_examples', 'external_general_text'}
    return sorted({str(row['source_category']) for row in rows if str(row['source_category']) not in allowed})
