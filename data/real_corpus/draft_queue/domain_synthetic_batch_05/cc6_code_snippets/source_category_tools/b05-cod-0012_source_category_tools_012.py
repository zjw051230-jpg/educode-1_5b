...
# learning_objective: Merge partial counters from multiple shards into one report.
...
TOPIC_ID = 'B05-COD-0012'
TITLE = 'Merge shard-level source counts'
...
def topic_action(rows: list[dict[str, object]]) -> object:
    shards = [count_by_category(rows[:2]), count_by_category(rows[2:]) ]
