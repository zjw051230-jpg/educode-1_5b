...
# learning_objective: Sort real category counts by descending frequency.
...
TOPIC_ID = 'B05-COD-0013'
TITLE = 'Sort categories by frequency'
...
def topic_action(rows: list[dict[str, object]]) -> object:
    return sorted(count_by_category(rows).items(), key=lambda item: (-item[1], item[0]))
