...
# learning_objective: Count how each worker contributes to each category.
...
TOPIC_ID = 'B05-COD-0014'
TITLE = 'Count categories per worker'
...
def topic_action(rows: list[dict[str, object]]) -> object:
    ...
    for row in rows:
        grouped[str(row['worker_id'])][str(row['source_category'])] += 1
