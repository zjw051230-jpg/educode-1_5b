...
# learning_objective: Raise alerts when expected categories are absent.
...
TOPIC_ID = 'B05-COD-0015'
TITLE = 'Build missing-category alerts'
...
def topic_action(rows: list[dict[str, object]]) -> object:
    expected = {'synthetic_examples', 'external_general_text', 'internal_notes'}
    seen = {str(row['source_category']) for row in rows}
