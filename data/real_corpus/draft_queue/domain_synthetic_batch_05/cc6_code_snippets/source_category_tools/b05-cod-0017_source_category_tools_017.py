...
# learning_objective: Return coverage information for every split.
...
TOPIC_ID = 'B05-COD-0017'
TITLE = 'Summarize category coverage by split'
...
def topic_action(rows: list[dict[str, object]]) -> object:
    ...
    for row in rows:
        coverage[str(row['split'])].add(str(row['source_category']))
    return {split: sorted(values) for split, values in sorted(coverage.items())}
