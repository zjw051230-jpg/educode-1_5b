...
# learning_objective: Attach source counts to synthetic manifest-style rows.
...
TOPIC_ID = 'B05-COD-0019'
TITLE = 'Project category counts into manifest rows'
...
def topic_action(rows: list[dict[str, object]]) -> object:
    return [{'topic_id': TOPIC_ID, 'source_category': key, 'count': value} for key, value in count_by_category(rows).items()]
