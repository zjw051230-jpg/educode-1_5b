...
# learning_objective: Apply split/category filters before final counting.
...
TOPIC_ID = 'B05-COD-0020'
TITLE = 'Filter rows before category counting'
...
def topic_action(rows: list[dict[str, object]]) -> object:
    val_rows = [row for row in rows if str(row['split']) == 'val']
    return count_by_category(val_rows)
