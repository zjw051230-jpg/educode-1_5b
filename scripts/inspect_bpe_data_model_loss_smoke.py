...
def main() -> int:
    ...
    if not train_texts:
        print("failure: no train texts found in processed synthetic seed corpus")
        return 1
    ...
    if not batches:
        print("failure: no full batch could be created from the BPE train corpus")
        return 1
    ...
    baseline = math.log(config["model"]["vocab_size"])
    ...
    print(f"baseline note: random-init loss is often near ln(vocab_size) ~= {baseline:.2f} for vocab_size={config['model']['vocab_size']}")
    print(f"device: {device}")

    if loss_finite:
        print("success: BPE processed-data forward/loss smoke passed")
        return 0
