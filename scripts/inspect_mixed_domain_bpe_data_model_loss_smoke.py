...
PROCESSED_PATH = PROJECT_ROOT / "data" / "real_corpus" / "processed" / "mixed_domain_external.processed.jsonl"
TRAIN_SPLIT = "train"
EXPECTED_SOURCE_CATEGORIES = {"synthetic_examples", "external_general_text"}


def load_train_texts() -> tuple[list[str], dict[str, int]]:
    ...
    with PROCESSED_PATH.open("r", encoding="utf-8") as handle:
        for line in handle:
            ...
            train_source_counts[source_category] = train_source_counts.get(source_category, 0) + 1
    return train_texts, train_source_counts
...
def main() -> int:
    ...
    train_texts, train_source_counts = load_train_texts()
    if not train_texts:
        print("failure: no train texts found in processed mixed domain/external corpus")
        return 1

    observed_source_categories = set(train_source_counts)
    if observed_source_categories != EXPECTED_SOURCE_CATEGORIES:
        print(
            "failure: expected train source categories "
            f"{sorted(EXPECTED_SOURCE_CATEGORIES)} but found {sorted(observed_source_categories)}"
        )
        return 1

    external_count = train_source_counts.get("external_general_text", 0)
    synthetic_count = train_source_counts.get("synthetic_examples", 0)
    if external_count <= 0:
        print("failure: mixed corpus train split is missing external_general_text records")
        return 1
    if external_count >= synthetic_count:
        print(
            "failure: external_general_text must remain supplement only in the train split "
            f"but observed external={external_count} synthetic={synthetic_count}"
        )
        return 1
    ...
    if not batches:
        print("failure: no full batch could be created from the mixed/domain BPE train corpus")
        return 1
    ...
    print(f"train source counts: {json.dumps(train_source_counts, ensure_ascii=False, sort_keys=True)}")
    ...
    if loss_finite:
        print("success: mixed/domain BPE processed-data model/loss smoke passed")
        return 0
