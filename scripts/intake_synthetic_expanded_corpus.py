...
RAW_DIR = PROJECT_ROOT / "data" / "real_corpus" / "raw" / "synthetic_expanded"
MANIFEST_PATH = PROJECT_ROOT / "data" / "real_corpus" / "metadata" / "source_manifest.synthetic_expanded.jsonl"
PROCESSED_PATH = PROJECT_ROOT / "data" / "real_corpus" / "processed" / "synthetic_expanded.processed.jsonl"
TRAIN_PATH = PROJECT_ROOT / "data" / "real_corpus" / "splits" / "synthetic_expanded.train.jsonl"
VAL_PATH = PROJECT_ROOT / "data" / "real_corpus" / "splits" / "synthetic_expanded.val.jsonl"
DROPPED_PATH = PROJECT_ROOT / "data" / "real_corpus" / "metadata" / "synthetic_expanded.dropped_files.jsonl"
SUMMARY_PATH = PROJECT_ROOT / "data" / "real_corpus" / "metadata" / "synthetic_expanded.intake_summary.json"

ALLOWED_SUFFIXES = {".md", ".txt", ".py"}
EXPECTED_SOURCE_ID = "source_synthetic_expanded_000001"
SPLIT_SEED = 1337
VAL_RATIO = 0.10
...
def list_candidate_files() -> list[Path]:
    return sorted(
        [
            ...
            and not (path.parent == RAW_DIR and path.name == "README.md")
        ],
...
def main() -> int:
    ...
    summary = {
        ...
        "split_ratio": {"train": 0.90, "val": 0.10},
        "validation": {
    ...
    print(f"train_output: {TRAIN_PATH.relative_to(PROJECT_ROOT).as_posix()}")
    print(f"val_output: {VAL_PATH.relative_to(PROJECT_ROOT).as_posix()}")
