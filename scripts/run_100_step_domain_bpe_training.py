...
CONFIG_PATH = PROJECT_ROOT / "configs" / "windows" / "bpe_domain_8k_smoke.json"
TRAIN_SPLIT_PATH = PROJECT_ROOT / "data" / "real_corpus" / "splits" / "synthetic_expanded.train.jsonl"
VAL_SPLIT_PATH = PROJECT_ROOT / "data" / "real_corpus" / "splits" / "synthetic_expanded.val.jsonl"
MAX_STEPS = 100
EVAL_INTERVAL = 10
...
def main() -> int:
    ...
    stage = "windows_cuda"
    short_name = "100_step_domain_bpe_training"
    ...
    metadata_path = write_run_metadata(
        ...
        env=env,
        status="running",
        notes="D11.4 bounded 100-step domain BPE training in progress",
    )
    ...
    metadata["notes"] = "D11.4 bounded 100-step domain BPE training complete"
    ...
    summary_markdown = build_summary_markdown(
        {
            "run_id": run_id,
            "goal": "Validate a strictly bounded 100-step domain BPE training run with periodic validation on the domain synthetic expanded corpus.",
            ...
            "generation_preview": "",
            "notes": "Domain synthetic corpus only; domain BPE tokenizer only; bounded 100-step local training; checkpoint reload sanity check passed. No model quality claims."
            if checkpoint_reload_match
            else "Domain synthetic corpus only; domain BPE tokenizer only; bounded 100-step local training; checkpoint reload sanity check failed. No model quality claims.",
            "next_step": "Review the bounded 100-step domain BPE training artifacts before any longer or non-local run.",
        }
    )
