...
MAX_STEPS = 100
EVAL_INTERVAL = 10
DEFAULT_PROMPT = "def hello_world():"
...
def main() -> int:
    ...
    generation_tokenizer = BPETokenizerAdapter(tokenizer)
    ...
    sequence_length = min(config["model"]["context_length"], configured_sequence_length, 64)
    ...
    stage = "windows_cuda"
    short_name = "100_step_small_real_data_training"
    ...
    metadata_path = write_run_metadata(
        ...
        env=env,
        status="running",
        notes="T7.4 bounded 100-step synthetic-seed small training in progress",
    )
    ...
    checkpoints_manifest_path = run_dir / "checkpoints_manifest.json"
    ...
    prompts = generation_config.get("prompts") if isinstance(generation_config.get("prompts"), list) else []
    prompt = prompts[0] if prompts and isinstance(prompts[0], str) and prompts[0].strip() else DEFAULT_PROMPT
    max_new_tokens = generation_config.get("max_new_tokens", DEFAULT_MAX_NEW_TOKENS)
    ...
    top_k = generation_config.get("top_k", DEFAULT_TOP_K)
    ...
    write_json(
        checkpoints_manifest_path,
        {
            ...
            "reload_match": checkpoint_reload_match,
            "timestamp": datetime.now().isoformat(timespec="seconds"),
        },
    )
    ...
    success = (
        ...
        and min_train_loss is not None
        and max_train_loss is not None
        and mean_train_loss is not None
        and min_val_loss is not None
        and max_val_loss is not None
        and mean_val_loss is not None
        and final_grad_norm is not None
        and metrics_rows == MAX_STEPS
        and len(val_losses) == MAX_STEPS // EVAL_INTERVAL
        and loss_all_finite
        and grad_all_finite
        ...
        and checkpoints_manifest_path.exists()
    ...
    metadata["notes"] = "T7.4 bounded 100-step synthetic-seed small training complete"
    ...
    summary_markdown = build_summary_markdown(
        {
            "run_id": run_id,
            "goal": "Validate a strictly bounded 100-step synthetic-seed training run with periodic validation on Windows.",
            ...
            "key_metrics": {
                ...
                "min_train_loss": round(min_train_loss, 6) if min_train_loss is not None else None,
                "max_train_loss": round(max_train_loss, 6) if max_train_loss is not None else None,
                "mean_train_loss": round(mean_train_loss, 6) if mean_train_loss is not None else None,
                "min_val_loss": round(min_val_loss, 6) if min_val_loss is not None else None,
                "max_val_loss": round(max_val_loss, 6) if max_val_loss is not None else None,
                "mean_val_loss": round(mean_val_loss, 6) if mean_val_loss is not None else None,
            ...
            "notes": "Synthetic seed corpus only; observed BPE vocab is 1174; bounded 100-step local training; checkpoint reload sanity check passed."
            if checkpoint_reload_match
            else "Synthetic seed corpus only; observed BPE vocab is 1174; bounded 100-step local training; checkpoint reload sanity check failed.",
            "next_step": "Review the bounded 100-step small-training artifacts before any longer or external-data run.",
        }
    )
    write_text(summary_path, summary_markdown)

    success = success and summary_path.exists()
    ...
    print(f"min_train_loss: {min_train_loss:.6f}" if min_train_loss is not None else "min_train_loss: None")
    print(f"max_train_loss: {max_train_loss:.6f}" if max_train_loss is not None else "max_train_loss: None")
    print(f"mean_train_loss: {mean_train_loss:.6f}" if mean_train_loss is not None else "mean_train_loss: None")
    print(f"min_val_loss: {min_val_loss:.6f}" if min_val_loss is not None else "min_val_loss: None")
    print(f"max_val_loss: {max_val_loss:.6f}" if max_val_loss is not None else "max_val_loss: None")
    print(f"mean_val_loss: {mean_val_loss:.6f}" if mean_val_loss is not None else "mean_val_loss: None")
    print(f"loss_all_finite: {loss_all_finite}")
    print(f"grad_all_finite: {grad_all_finite}")
    print(f"final_grad_norm: {final_grad_norm:.6f}" if final_grad_norm is not None else "final_grad_norm: None")
    print(f"tokens_seen: {total_tokens_seen}")
    print(f"elapsed_seconds: {total_elapsed_seconds:.6f}")
    print(
        f"approximate tokens/sec: {approximate_tokens_per_sec:.6f}"
        if approximate_tokens_per_sec is not None
        else "approximate tokens/sec: None"
    )
    ...
    print(f"validation rows: {len(val_losses)}")
