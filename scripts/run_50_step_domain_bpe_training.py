...
def main() -> int:
    ...
    stage = "windows_cuda"
    short_name = "50_step_domain_bpe_training"
    ...
    metadata_path = write_run_metadata(
        ...
        env=env,
        status="running",
        notes="D11.1 bounded 50-step domain BPE training in progress",
    )
    ...
    metadata["notes"] = "D11.1 bounded 50-step domain BPE training complete"
    ...
    summary_markdown = build_summary_markdown(
        {
            "run_id": run_id,
            "goal": "Validate a strictly bounded 50-step domain BPE training run with periodic validation on the domain synthetic expanded corpus.",
            ...
            "generation_preview": "",
            "notes": "Domain synthetic corpus only; domain BPE tokenizer only; bounded 50-step local training; checkpoint reload sanity check passed. No model quality claims."
            if checkpoint_reload_match
            else "Domain synthetic corpus only; domain BPE tokenizer only; bounded 50-step local training; checkpoint reload sanity check failed. No model quality claims.",
            "next_step": "Review the bounded domain BPE training artifacts before any longer or non-local run.",
        }
    )
