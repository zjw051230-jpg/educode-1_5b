...
def main() -> int:
    ...
    stage = "windows_cuda"
    short_name = "50_step_expanded_bpe_training"
    ...
    metadata_path = write_run_metadata(
        ...
        env=env,
        status="running",
        notes="D6.1 bounded 50-step expanded BPE training in progress",
    )
    ...
    metadata["notes"] = "D6.1 bounded 50-step expanded BPE training complete"
    ...
    summary_markdown = build_summary_markdown(
        {
            "run_id": run_id,
            "goal": "Validate a strictly bounded 50-step expanded BPE training run with periodic validation on the expanded synthetic corpus.",
            ...
            "generation_preview": "",
            "notes": "Expanded synthetic corpus only; expanded BPE tokenizer only; bounded 50-step local training; checkpoint reload sanity check passed. No model quality claims."
            if checkpoint_reload_match
            else "Expanded synthetic corpus only; expanded BPE tokenizer only; bounded 50-step local training; checkpoint reload sanity check failed. No model quality claims.",
            "next_step": "Review the bounded expanded BPE training artifacts before any longer or non-local run.",
        }
    )
