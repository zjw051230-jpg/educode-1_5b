...
def main() -> int:
    ...
    stage = "windows_cuda"
    short_name = "50_step_mixed_domain_bpe_training"
    ...
    metadata_path = write_run_metadata(
        ...
        env=env,
        status="running",
        notes="D16.1 bounded 50-step mixed/domain BPE training in progress",
    )
    ...
    metadata["notes"] = "D16.1 bounded 50-step mixed/domain BPE training complete"
    ...
    summary_markdown = build_summary_markdown(
        {
            "run_id": run_id,
            "goal": "Validate a strictly bounded 50-step mixed/domain BPE training run with periodic validation on the approved mixed corpus.",
            "hardware": f"{device} / {config['hardware'].get('target')}",
            "config_path": str(snapshot_path),
            "result": "success" if success else "failed",
            "key_metrics": summary_data,
            "generation_preview": "",
            "notes": "Approved mixed_domain_external corpus only; mixed/domain BPE tokenizer only; external_general_text remains supplement only; bounded 50-step local training; checkpoint reload sanity check passed. No model quality claims."
            if checkpoint_reload_match
            else "Approved mixed_domain_external corpus only; mixed/domain BPE tokenizer only; external_general_text remains supplement only; bounded 50-step local training; checkpoint reload sanity check failed. No model quality claims.",
