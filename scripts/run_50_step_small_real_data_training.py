...
def main() -> int:
    ...
    stage = "windows_cuda"
    short_name = "50_step_small_real_data_training"
    ...
    metadata_path = write_run_metadata(
        ...
        env=env,
        status="running",
        notes="T7.1 bounded 50-step synthetic-seed small training in progress",
    )
    ...
    metadata["notes"] = "T7.1 bounded 50-step synthetic-seed small training complete"
    ...
    summary_markdown = build_summary_markdown(
        {
            "run_id": run_id,
            "goal": "Validate a strictly bounded 50-step synthetic-seed training run with periodic validation on Windows.",
            ...
            "notes": "Synthetic seed corpus only; observed BPE vocab is 1174; bounded 50-step local training; checkpoint reload sanity check passed."
            if checkpoint_reload_match
            else "Synthetic seed corpus only; observed BPE vocab is 1174; bounded 50-step local training; checkpoint reload sanity check failed.",
            "next_step": "Review the bounded small real-data training artifacts before any longer or external-data run.",
        }
    )
