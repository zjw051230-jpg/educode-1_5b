...
def main() -> int:
    ...
    stage = "windows_cuda"
    short_name = "bounded_50_step_toy"
    ...
    metadata_path = write_run_metadata(
        ...
        env=env,
        status="running",
        notes="W11.4 bounded 50-step toy training in progress",
    )
    ...
    for step in range(1, max_steps + 1):
        ...
        print(f"step {step}: loss={current_loss:.6f} grad_norm={final_grad_norm:.6f}")
    ...
    metadata["notes"] = "W11.4 bounded 50-step toy training complete"
    ...
    summary_markdown = build_summary_markdown(
        {
            "run_id": run_id,
            "goal": "Validate a strictly bounded 50-step toy-data training run on Windows.",
            ...
            "key_metrics": {
                ...
                "min_loss": round(min_loss, 6) if min_loss is not None else None,
            ...
            "notes": "Toy data only; ByteTokenizer temporary path; no validation set; checkpoint reload sanity check passed."
            if checkpoint_reload_match
            else "Toy data only; ByteTokenizer temporary path; no validation set; checkpoint reload sanity check failed.",
            "next_step": "W11.5 bounded 50-step toy training review",
        }
    )
    ...
    print(f"min_loss: {min_loss:.6f}" if min_loss is not None else "min_loss: None")
    print(f"max_loss: {max_loss:.6f}" if max_loss is not None else "max_loss: None")
    print(f"mean_loss: {mean_loss:.6f}" if mean_loss is not None else "mean_loss: None")
