...
CORPUS_PATH = PROJECT_ROOT / "data" / "real_corpus" / "processed" / "synthetic_seed.processed.jsonl"
ARTIFACT_DIR = PROJECT_ROOT / "tokenizers" / "educode_bpe_8k"
TOKENIZER_NAME = "educode_bpe_8k"
...
def main() -> int:
    ...
    tokenizer_config = {
        ...
        "artifact_path": "tokenizers/educode_bpe_8k",
        "tokenizer_json": "tokenizers/educode_bpe_8k/tokenizer.json",
        "corpus_path": "data/real_corpus/processed/synthetic_seed.processed.jsonl",
        "splits_used": [TRAIN_SPLIT],
        "input_docs": input_docs,
        "train_docs_used": train_docs_used,
        "is_production_tokenizer": False,
        "notes": "Formal-path smoke tokenizer trained only on the processed synthetic seed corpus.",
    }
    ...
    readme_text = f"""# EduCode BPE 8k

This directory stores the `educode_bpe_8k` tokenizer artifact trained only on the processed synthetic seed corpus.
It is a formal-path smoke artifact for tokenizer integration work.
...
Corpus path: `data/real_corpus/processed/synthetic_seed.processed.jsonl`
...
"""
    ...
    if observed_vocab_size < TARGET_VOCAB_SIZE:
        print("warning: observed vocab size is below target because the current corpus is very small")
