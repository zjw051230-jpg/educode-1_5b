...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate A800/A100 1000-step public16k run artifacts after execution.")
...
def main() -> int:
    ...
    if summary:
        checkpoint_path_value = summary.get("checkpoint_path")
        ...
        require(summary.get("max_steps") == EXPECTED_MAX_STEPS, "summary max_steps must be 1000", blockers)
        require(summary.get("metrics_rows") == EXPECTED_MAX_STEPS, "summary metrics_rows must be 1000", blockers)
        require(summary.get("validation_rows", 0) >= MIN_VALIDATION_ROWS, "summary validation_rows must be at least 10", blockers)
        require(metrics_rows_actual == EXPECTED_MAX_STEPS, "metrics.jsonl actual rows must be 1000", blockers)
        require(
        ...
        if checkpoint_path is None:
            ...
            require(path_is_under(checkpoint_path, output_dir), "checkpoint_path must start with output_dir", blockers)
    ...
    result = {
        ...
        "checkpoint_path": summary.get("checkpoint_path") if summary else None,
        "checkpoint_path_starts_with_output_dir": bool(
            isinstance(summary.get("checkpoint_path"), str)
            and path_is_under(resolve_repo_path(str(summary.get("checkpoint_path"))), output_dir)
        )
        if summary
        else None,
